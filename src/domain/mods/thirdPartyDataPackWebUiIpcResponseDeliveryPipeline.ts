...
export interface CreateThirdPartyDataPackWebUiIpcResponseDeliveryPipelineOptions {
...
export const createThirdPartyDataPackWebUiIpcResponseDeliveryPipeline = (
  options: CreateThirdPartyDataPackWebUiIpcResponseDeliveryPipelineOptions = {}
  ...
  const readResultNormalizationPreflightOnce = async() => {
    resultNormalizationPreflight ??= Promise.resolve().then(() => {
      if (options.readResultNormalizationPreflight === undefined) {
        throw new Error('third-party Web UI/IPC response delivery pipeline missing result normalization preflight reader')
      }
  ...
  const readOutcomeHandoffOnce = async() => {
    outcomeHandoff ??= Promise.resolve().then(() => {
      if (options.readPostCommitVerificationUiIpcOutcomeHandoff === undefined) {
        throw new Error('third-party Web UI/IPC response delivery pipeline missing post-commit UI/IPC outcome handoff reader')
      }
  ...
  const readPlatformSplitContract = async():
    ...
    if (postCommitVerificationUiIpcOutcomeHandoff.outcome === undefined) {
      throw new Error('third-party Web UI/IPC response delivery pipeline missing path-free UI/IPC outcome source')
    }
  ...
  const readWebResponseDeliveryStartupGateHandoff = async() => {
    ...
    const webResponseDelivery = await deliverThirdPartyDataPackWebResponseDeliverySinkAdapter({
    ...
    return buildThirdPartyDataPackWebResponseDeliveryStartupGateHandoff({
      webResponseDelivery,
      startupGateHandoffPreflight
    })
  }

  const readWebResponseDeliveryStartupGateHandoffSource =
    createThirdPartyDataPackWebResponseDeliveryStartupGateHandoffSource({
      enabled: options.enabled,
...
export const thirdPartyDataPackWebUiIpcResponseDeliveryPipeline =
