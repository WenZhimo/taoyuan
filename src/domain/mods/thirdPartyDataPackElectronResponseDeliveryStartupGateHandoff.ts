...
export const THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND =
  'electron-response-delivery-startup-gate-handoff'
export const THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE =
  'readonly-electron-response-delivery-startup-gate-handoff'

export type ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffStatus =
  | 'ready'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheckId =
  | 'electron-response-delivery-delivered'
  | 'electron-delivery-acknowledgement-present'
...
export interface ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck {
  readonly id: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheckId
  readonly status: 'satisfied' | 'skipped' | 'blocked'
  readonly reason: string
}

export interface ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffEffectSummary {
...
export interface ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffResult {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE
  readonly platform: 'electron'
  readonly status: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffStatus
  readonly electronResponseDeliveryStatus: ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult['status']
  readonly startupGateHandoffPreflightStatus: ThirdPartyDataPackStartupGateHandoffPreflightResult['status']
  readonly reason: string
  readonly electronResponseDeliveryStartupGateHandoff: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffStatus
  ...
  readonly effects: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffEffectSummary
}

export interface BuildThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffOptions {
  readonly electronResponseDelivery: ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult
...
const safeDiagnostic = (
  ...
  return Object.freeze({
    ...
    stage: readOwnStringField(diagnostic, 'stage')
      ?? 'third-party.electron-response-delivery-startup-gate-handoff.diagnostic-copy',
...
const check = (
  id: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheckId,
  status: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck['status'],
  reason: string
): ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck => Object.freeze({ id, status, reason })

const skippedChecks = (
  reason: string
): readonly ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck[] => Object.freeze([
  'electron-response-delivery-delivered',
  'electron-delivery-acknowledgement-present',
  'startup-gate-preflight-deferred',
  'startup-gate-preflight-prepared',
  ...
  id as ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheckId,
  'skipped',
  reason
)))
...
const packageSummaryConsistent = (
  delivery: ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult,
...
const electronDeliveryEffectsIntact = (
  delivery: ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult,
  envelopeKind: ThirdPartyDataPackUiIpcResultEnvelopeOutcomeKind | undefined
): boolean => readOwnBooleanField(delivery, 'electronResponseDelivered') === true
  && readOwnBooleanField(delivery, 'uiIpcResponseDeliveryAllowed') === true
  && readOwnBooleanField(delivery, 'electronIpcAllowed') === true
  && readOwnBooleanField(delivery, 'electronResponseDeliveryAllowed') === true
...
const noStartupRuntimeOrWriteEffects = (
  ...
  startup: ThirdPartyDataPackStartupGateHandoffPreflightResult,
  envelopeKind: ThirdPartyDataPackUiIpcResultEnvelopeOutcomeKind | undefined
): boolean => electronDeliveryEffectsIntact(delivery, envelopeKind)
...
const buildChecks = (
  ...
  check(
    ...
    readOwnStringField(delivery, 'status') === 'delivered'
      && readOwnBooleanField(delivery, 'electronResponseDelivered') === true
      ? 'satisfied'
      : 'blocked',
    'Electron response delivery must already have delivered the path-free envelope through the fixed Electron sink.'
  ),
  check(
    'electron-delivery-acknowledgement-present',
    acknowledgement !== undefined ? 'satisfied' : 'blocked',
    'Startup handoff can only consume a matching Electron delivery acknowledgement.'
  ),
  check(
    'startup-gate-preflight-deferred',
  ...
  check(
    ...
    'The delivered Electron envelope and startup handoff envelope must describe the same install result.'
  ),
  check(
    'package-summary-consistent',
    packageSummaryConsistent(delivery, startup) ? 'satisfied' : 'blocked',
    'Electron delivery and startup handoff must agree on selected packages, blocked packages, load order and totals.'
  ),
  check(
    'candidate-lockfile-consistent',
    candidateLockfileConsistent(delivery, startup) ? 'satisfied' : 'blocked',
    'Electron delivery and startup handoff must preserve the same candidate identity and lockfile hash.'
  ),
  check(
    'no-startup-runtime-or-write-effects',
    noStartupRuntimeOrWriteEffects(delivery, startup, deliveryEnvelope?.kind) ? 'satisfied' : 'blocked',
    'The bridge may consume Electron delivery acknowledgement and startup preflight evidence, but must not inherit startup, runtime, command, transaction, rollback or write effects.'
  )
])

const diagnosticsForBlockedChecks = (
  checks: readonly ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck[],
  packageId?: PackageId
  ...
  .map(currentCheck => commandDiagnostic(
    `third-party.electron-response-delivery-startup-gate-handoff.checks.${currentCheck.id}`,
    packageId
  )))
...
const baseResult = (
  status: ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffStatus,
  reason: string,
  delivery: ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult,
  startup: ThirdPartyDataPackStartupGateHandoffPreflightResult,
  checks: readonly ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffCheck[],
  ...
  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND,
    mode: THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE,
    platform: 'electron',
    status,
    electronResponseDeliveryStatus: readOwnStringField(delivery, 'status') as ThirdPartyDataPackElectronResponseDeliverySinkAdapterResult['status'],
    ...
    reason,
    electronResponseDeliveryStartupGateHandoff: status,
    readOnly: true,
    electronResponseDelivered: prepared,
...
export const buildThirdPartyDataPackElectronResponseDeliveryStartupGateHandoff = (
  options: BuildThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffOptions
): ThirdPartyDataPackElectronResponseDeliveryStartupGateHandoffResult => {
  const delivery = options.electronResponseDelivery
  ...
  if (deliveryStatus === 'skipped' || startupStatus === 'skipped') {
    const reason = readOwnStringField(delivery, 'reason')
      ...
      ?? 'electron response delivery startup handoff is not required because no third-party install result is pending'
    return baseResult(
      'skipped',
      reason,
      delivery,
  ...
  if (deliveryStatus === 'blocked' || startupStatus === 'blocked') {
    const reason = deliveryStatus === 'blocked'
      ? readOwnStringField(delivery, 'reason') ?? 'electron response delivery blocked before startup handoff'
      : readOwnStringField(startup, 'reason') ?? 'startup handoff preflight blocked before Electron response delivery handoff'
    return baseResult(
      'blocked',
      reason,
      delivery,
  ...
  if (blockedDiagnostics.length > 0) {
    return baseResult(
      'blocked',
      'electron response delivery startup handoff inputs are inconsistent',
      delivery,
      startup,
      checks,
      diagnostics,
  ...
  return baseResult(
    'ready',
    'electron response delivery acknowledgement is connected to startup gate handoff evidence; LauncherApp, GameApp, runtime reads and writes remain separate',
    delivery,
    startup,
    checks,
    diagnostics,
