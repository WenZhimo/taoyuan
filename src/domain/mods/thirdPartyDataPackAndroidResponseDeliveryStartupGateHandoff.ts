...
export const THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND =
  'android-response-delivery-startup-gate-handoff'
export const THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE =
  'readonly-android-response-delivery-startup-gate-handoff'

export type ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffStatus =
  | 'ready'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheckId =
  | 'android-response-delivery-delivered'
  | 'android-delivery-acknowledgement-present'
...
export interface ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck {
  readonly id: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheckId
  readonly status: 'satisfied' | 'skipped' | 'blocked'
  readonly reason: string
}

export interface ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffEffectSummary {
...
export interface ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE
  readonly platform: 'android'
  readonly status: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffStatus
  readonly androidResponseDeliveryStatus: ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult['status']
  readonly startupGateHandoffPreflightStatus: ThirdPartyDataPackStartupGateHandoffPreflightResult['status']
  readonly reason: string
  readonly androidResponseDeliveryStartupGateHandoff: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffStatus
  ...
  readonly effects: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffEffectSummary
}

export interface BuildThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffOptions {
  readonly androidResponseDelivery: ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult
...
const safeDiagnostic = (
  ...
  return Object.freeze({
    ...
    stage: readOwnStringField(diagnostic, 'stage')
      ?? 'third-party.android-response-delivery-startup-gate-handoff.diagnostic-copy',
...
const check = (
  id: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheckId,
  status: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck['status'],
  reason: string
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck => Object.freeze({ id, status, reason })

const skippedChecks = (
  reason: string
): readonly ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck[] => Object.freeze([
  'android-response-delivery-delivered',
  'android-delivery-acknowledgement-present',
  'startup-gate-preflight-deferred',
  'startup-gate-preflight-prepared',
  ...
  id as ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheckId,
  'skipped',
  reason
)))
...
const packageSummaryConsistent = (
  delivery: ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult,
...
const androidDeliveryEffectsIntact = (
  delivery: ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult,
  envelopeKind: ThirdPartyDataPackUiIpcResultEnvelopeOutcomeKind | undefined
): boolean => readOwnBooleanField(delivery, 'androidResponseDelivered') === true
  && readOwnBooleanField(delivery, 'uiIpcResponseDeliveryAllowed') === true
  && readOwnBooleanField(delivery, 'androidUiBridgeAllowed') === true
  && readOwnBooleanField(delivery, 'androidResponseDeliveryAllowed') === true
  ...
  && readOwnBooleanField(delivery, 'webUiBridgeAllowed') === false
  && readOwnBooleanField(delivery, 'webResponseDeliveryAllowed') === false
...
const noStartupRuntimeOrWriteEffects = (
  ...
  startup: ThirdPartyDataPackStartupGateHandoffPreflightResult,
  envelopeKind: ThirdPartyDataPackUiIpcResultEnvelopeOutcomeKind | undefined
): boolean => androidDeliveryEffectsIntact(delivery, envelopeKind)
...
const buildChecks = (
  ...
  check(
    ...
    readOwnStringField(delivery, 'status') === 'delivered'
      && readOwnBooleanField(delivery, 'androidResponseDelivered') === true
      ? 'satisfied'
      : 'blocked',
    'Android response delivery must already have delivered the path-free envelope through the fixed Android sink.'
  ),
  check(
    'android-delivery-acknowledgement-present',
    acknowledgement !== undefined ? 'satisfied' : 'blocked',
    'Startup handoff can only consume a matching Android delivery acknowledgement.'
  ),
  check(
    'startup-gate-preflight-deferred',
  ...
  check(
    ...
    'The delivered Android envelope and startup handoff envelope must describe the same install result.'
  ),
  check(
    'package-summary-consistent',
    packageSummaryConsistent(delivery, startup) ? 'satisfied' : 'blocked',
    'Android delivery and startup handoff must agree on selected packages, blocked packages, load order and totals.'
  ),
  check(
    'candidate-lockfile-consistent',
    candidateLockfileConsistent(delivery, startup) ? 'satisfied' : 'blocked',
    'Android delivery and startup handoff must preserve the same candidate identity and lockfile hash.'
  ),
  check(
    'no-startup-runtime-or-write-effects',
    noStartupRuntimeOrWriteEffects(delivery, startup, deliveryEnvelope?.kind) ? 'satisfied' : 'blocked',
    'The bridge may consume Android delivery acknowledgement and startup preflight evidence, but must not inherit startup, runtime, command, transaction, rollback or write effects.'
  )
])

const diagnosticsForBlockedChecks = (
  checks: readonly ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck[],
  packageId?: PackageId
  ...
  .map(currentCheck => commandDiagnostic(
    `third-party.android-response-delivery-startup-gate-handoff.checks.${currentCheck.id}`,
    packageId
  )))
...
const createEffectSummary = (
  prepared: boolean,
  envelopeKind: ThirdPartyDataPackUiIpcResultEnvelopeOutcomeKind | undefined
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffEffectSummary => ({
  ...
  androidUiResponsePublished: prepared,
  ...
  androidResponseDeliveryAcknowledgementConsumed: prepared,
...
const baseResult = (
  status: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffStatus,
  reason: string,
  delivery: ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult,
  startup: ThirdPartyDataPackStartupGateHandoffPreflightResult,
  checks: readonly ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffCheck[],
  ...
  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_KIND,
    mode: THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_MODE,
    platform: 'android',
    status,
    androidResponseDeliveryStatus: readOwnStringField(delivery, 'status') as ThirdPartyDataPackAndroidResponseDeliverySinkAdapterResult['status'],
    ...
    reason,
    androidResponseDeliveryStartupGateHandoff: status,
    readOnly: true,
    androidResponseDelivered: prepared,
...
export const buildThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoff = (
  options: BuildThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffOptions
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult => {
  const delivery = options.androidResponseDelivery
  ...
  if (deliveryStatus === 'skipped' || startupStatus === 'skipped') {
    const reason = readOwnStringField(delivery, 'reason')
      ...
      ?? 'android response delivery startup handoff is not required because no third-party install result is pending'
    return baseResult(
      'skipped',
      reason,
      delivery,
  ...
  if (deliveryStatus === 'blocked' || startupStatus === 'blocked') {
    const reason = deliveryStatus === 'blocked'
      ? readOwnStringField(delivery, 'reason') ?? 'android response delivery blocked before startup handoff'
      : readOwnStringField(startup, 'reason') ?? 'startup handoff preflight blocked before Android response delivery handoff'
    return baseResult(
      'blocked',
      reason,
      delivery,
  ...
  if (blockedDiagnostics.length > 0) {
    return baseResult(
      'blocked',
      'android response delivery startup handoff inputs are inconsistent',
      delivery,
      startup,
      checks,
      diagnostics,
  ...
  return baseResult(
    'ready',
    'android response delivery acknowledgement is connected to startup gate handoff evidence; LauncherApp, GameApp, runtime reads and writes remain separate',
    delivery,
    startup,
    checks,
    diagnostics,
