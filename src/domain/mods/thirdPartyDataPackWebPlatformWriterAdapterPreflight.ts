...
export const THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_KIND =
  'third-party-web-platform-writer-adapter-preflight'
export const THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_MODE =
  'default-disabled-web-platform-writer-adapter-preflight'

export type ThirdPartyDataPackWebPlatformWriterAdapterPreflightStatus =
  | 'ready'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackWebPlatformWriterAdapterRequirementStatus =
  | 'required'
  | 'skipped'
  | 'blocked'

export interface ThirdPartyDataPackWebPlatformWriterAdapterRequirement {
  readonly id: string
  readonly status: ThirdPartyDataPackWebPlatformWriterAdapterRequirementStatus
  readonly reason: string
}

export interface ThirdPartyDataPackWebPlatformWriterAdapterCheck {
  readonly id: string
  readonly status: 'satisfied' | 'blocked'
  readonly reason: string
}

export interface ThirdPartyDataPackWebPlatformWriterAdapterPreflightEffectSummary {
  ...
  readonly webPlatformWriterAdapterReady: boolean
  readonly realWebPlatformWriterHostCalled: false
  readonly webPlatformWriterConnected: false
  readonly webIndexedDbStorageResolved: false
...
export interface ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic {
...
export interface ThirdPartyDataPackWebPlatformWriterAdapterPreflightResult {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_MODE
  readonly status: ThirdPartyDataPackWebPlatformWriterAdapterPreflightStatus
  readonly reason: string
  ...
  readonly webConnectionRequirementStatus?: ThirdPartyDataPackPlatformWriterConnectionRequirement['status']
  ...
  readonly webRequirements: readonly ThirdPartyDataPackWebPlatformWriterAdapterRequirement[]
  readonly checks: readonly ThirdPartyDataPackWebPlatformWriterAdapterCheck[]
  readonly diagnostics: readonly ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic[]
  readonly effects: ThirdPartyDataPackWebPlatformWriterAdapterPreflightEffectSummary
}

export interface CreateThirdPartyDataPackWebPlatformWriterAdapterPreflightOptions {
  readonly enabled?: boolean
  readonly readPlatformWriterConnectionPreflight?: () =>
    Awaitable<ThirdPartyDataPackPlatformWriterConnectionPreflightResult>
}

export class ThirdPartyDataPackWebPlatformWriterAdapterPreflightBlockedError extends Error {
  readonly result: ThirdPartyDataPackWebPlatformWriterAdapterPreflightResult

  constructor(result: ThirdPartyDataPackWebPlatformWriterAdapterPreflightResult) {
    super('third-party Web platform writer adapter preflight blocked command continuation')
    this.name = 'ThirdPartyDataPackWebPlatformWriterAdapterPreflightBlockedError'
    this.result = result
  }
}
...
const safeDiagnostic = (
  diagnostic: object
): ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic => {
  ...
  return Object.freeze({
    ...
    stage: readOwnStringField(diagnostic, 'stage')
      ?? 'third-party.web-platform-writer-adapter-preflight.diagnostic-copy',
...
): readonly ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic[] => {
  if (!Array.isArray(diagnostics)) return Object.freeze([])
  const length = readArrayLength(diagnostics)
  if (length === undefined) return Object.freeze([])

  const result: ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic[] = []
  for (let index = 0; index < length; index += 1) {
    let descriptor: PropertyDescriptor | undefined
    try {
...
const commandDiagnostic = (
  stage: string,
  packageId?: PackageId
): ThirdPartyDataPackWebPlatformWriterAdapterSafeDiagnostic => Object.freeze({
...
const webConnectionRequirement = (
  ...
  cloneUpstreamRequirements(readOwnDataField(source, 'requirements'))
    .find(requirement => requirement.platform === 'web')
...
const safeReadySource = (
  ...
  return readOwnStringField(source, 'status') === 'deferred'
    ...
    && webConnectionRequirement(source)?.status === 'required'
...
const adapterRequirement = (
  id: string,
  status: ThirdPartyDataPackWebPlatformWriterAdapterRequirementStatus,
  reason: string
): ThirdPartyDataPackWebPlatformWriterAdapterRequirement => Object.freeze({ id, status, reason })

const buildWebRequirements = (
  status: ThirdPartyDataPackWebPlatformWriterAdapterPreflightStatus
): readonly ThirdPartyDataPackWebPlatformWriterAdapterRequirement[] => {
  const requirementStatus: ThirdPartyDataPackWebPlatformWriterAdapterRequirementStatus =
  ...
  return Object.freeze([
    adapterRequirement(
      ...
      requirementStatus === 'required'
        ? 'Web writer must resolve userdata from IndexedDB-scoped browser persistence and never from renderer-supplied paths'
        : 'Web IndexedDB userdata root resolution is not active for this preflight state'
    ),
    adapterRequirement(
      'web-mod-lock-indexeddb-atomic-writer',
      requirementStatus,
      requirementStatus === 'required'
        ? 'Web writer must atomically persist the mod-lock record in IndexedDB with verification before command continuation'
        : 'Web IndexedDB mod-lock writer is not active for this preflight state'
    ),
    adapterRequirement(
      'web-settings-indexeddb-atomic-writer',
      requirementStatus,
      requirementStatus === 'required'
        ? 'Web writer must atomically persist the settings record in IndexedDB without touching saves or cache'
        : 'Web IndexedDB settings writer is not active for this preflight state'
    ),
    adapterRequirement(
      'web-path-free-storage-command-envelope',
      requirementStatus,
      requirementStatus === 'required'
        ? 'Web storage commands must use fixed-purpose envelopes and not accept renderer-supplied paths or browser storage handles'
        : 'Web storage command envelope is not active for this preflight state'
    )
  ])
}
...
const check = (
  id: string,
  status: ThirdPartyDataPackWebPlatformWriterAdapterCheck['status'],
  reason: string
): ThirdPartyDataPackWebPlatformWriterAdapterCheck => Object.freeze({ id, status, reason })

const buildChecks = (
  status: ThirdPartyDataPackWebPlatformWriterAdapterPreflightStatus,
  source?: ThirdPartyDataPackPlatformWriterConnectionPreflightResult
): readonly ThirdPartyDataPackWebPlatformWriterAdapterCheck[] => Object.freeze([
  check(
  ...
  check(
    'web-platform-writer-requirement-present',
    status === 'blocked' ? 'blocked' : 'satisfied',
    webConnectionRequirement(source)?.status === 'required'
      ? 'Web platform writer connection requirement is present and remains deferred'
      : 'Web platform writer connection is skipped until upstream persistent writes exist'
  ),
  check(
    'no-real-web-writer-called',
    'satisfied',
    'preflight does not call Web writer hosts or expose IndexedDB/browser storage objects or renderer-supplied paths'
  )
])
...
const effectSummary = (
  ...
  source?: ThirdPartyDataPackPlatformWriterConnectionPreflightResult
): ThirdPartyDataPackWebPlatformWriterAdapterPreflightEffectSummary => {
  ...
  return Object.freeze({
    ...
    webPlatformWriterAdapterReady: ready,
...
const baseResult = (
  ...
  const webRequirementStatus = upstreamRequirements.find(requirement => requirement.platform === 'web')?.status

  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_KIND,
    mode: THIRD_PARTY_DATA_PACK_WEB_PLATFORM_WRITER_ADAPTER_PREFLIGHT_MODE,
    ...
    platformWriterConnectionPreflightStatus: readOwnStringField(options.source, 'status') as
      ...
      | undefined,
    webConnectionRequirementStatus: webRequirementStatus,
    ...
    webRequirements: buildWebRequirements(options.status),
...
const evaluateWebPlatformWriterAdapterPreflight = async(
  options: CreateThirdPartyDataPackWebPlatformWriterAdapterPreflightOptions
): Promise<ThirdPartyDataPackWebPlatformWriterAdapterPreflightResult> => {
  if (options.enabled !== true) {
    return baseResult({
      status: 'skipped',
      reason: 'third-party Web platform writer adapter preflight is disabled by default',
      enabled: false,
      sourceCalled: false
    })
  }
  ...
  if (options.readPlatformWriterConnectionPreflight === undefined) {
    return baseResult({
      status: 'blocked',
      reason: 'third-party Web platform writer adapter preflight is enabled without a platform writer connection preflight',
      enabled: true,
      sourceCalled: false,
      diagnostics: [
        commandDiagnostic('third-party.web-platform-writer-adapter-preflight.missing-source')
      ]
    })
  }
  ...
  try {
    ...
    return baseResult({
      status: 'blocked',
      reason: 'third-party platform writer connection preflight failed before Web writer adapter preflight',
      enabled: true,
      sourceCalled: true,
      diagnostics: [
        commandDiagnostic('third-party.web-platform-writer-adapter-preflight.source-failed')
      ]
    })
  }
  ...
  if (safeSkippedSource(source)) {
    return baseResult({
      status: 'skipped',
      reason: 'third-party Web platform writer adapter is not required because platform writer connection was skipped',
      enabled: true,
      sourceCalled: true,
      source,
  ...
  if (safeReadySource(source)) {
    return baseResult({
      status: 'ready',
      reason: 'third-party Web platform writer adapter preflight accepted path-free writer requirements; real Web writer wiring remains deferred',
      enabled: true,
      sourceCalled: true,
      source,
      diagnostics: sourceDiagnostics
  ...
  const blockedDiagnostics = [
    ...
    ...(!pathFreeConnectionSource(source) || !connectionEffectsContained(source)
      ? [
          commandDiagnostic(
            'third-party.web-platform-writer-adapter-preflight.unsafe-source',
            targetPackageId
          )
        ]
      : []),
    commandDiagnostic(
      'third-party.web-platform-writer-adapter-preflight.connection-blocked',
      targetPackageId
    )
  ]
  ...
  return baseResult({
    status: 'blocked',
    reason: 'third-party Web platform writer adapter requires safe deferred Web writer requirements before real platform wiring may proceed',
    enabled: true,
    sourceCalled: true,
    source,
...
export const createThirdPartyDataPackWebPlatformWriterAdapterPreflight = (
  options: CreateThirdPartyDataPackWebPlatformWriterAdapterPreflightOptions = {}
): (() => Promise<ThirdPartyDataPackWebPlatformWriterAdapterPreflightResult>) => async() => {
  const result = await evaluateWebPlatformWriterAdapterPreflight(options)
  if (result.status === 'blocked') {
    throw new ThirdPartyDataPackWebPlatformWriterAdapterPreflightBlockedError(result)
  }
  return result
}

export const thirdPartyDataPackWebPlatformWriterAdapterPreflight =
