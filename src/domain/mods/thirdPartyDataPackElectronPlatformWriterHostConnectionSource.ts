...
export const THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_KIND =
  'third-party-electron-platform-writer-host-connection-source'
export const THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_MODE =
  'default-disabled-electron-platform-writer-host-connection-source'

export type ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceStatus =
  | 'connected'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackElectronPlatformWriterHostConnectionStatus =
  | 'accepted'
  | 'blocked'

export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionEnvelope {
  ...
  readonly electronRequirementIds: readonly string[]
}

export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionEffectSummary {
  readonly electronPlatformWriterHostCalled: boolean
  readonly electronPlatformWriterHostAccepted: boolean
  readonly electronPlatformWriterConnected: boolean
  readonly programDirectoryUserdataResolved: boolean
...
export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionResult {
  readonly status: ThirdPartyDataPackElectronPlatformWriterHostConnectionStatus
  readonly requestedCommandId?: 'install'
  readonly targetPackageId?: PackageId
  readonly selectedPackageIds?: readonly PackageId[]
  readonly blockedPackageIds?: readonly PackageId[]
  ...
  readonly effects: ThirdPartyDataPackElectronPlatformWriterHostConnectionEffectSummary
}

export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceEffectSummary {
  readonly electronPlatformWriterHostConnectionSourceCalled: boolean
  readonly electronPlatformWriterAdapterPreflightCalled: boolean
  readonly injectedElectronPlatformWriterHostCalled: boolean
  readonly electronPlatformWriterHostCalled: boolean
  readonly electronPlatformWriterHostAccepted: boolean
...
export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionSafeDiagnostic {
...
export interface ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_MODE
  readonly status: ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceStatus
  ...
  readonly appBootstrapContinuationAllowed: boolean
  readonly commandContinuationAllowed: boolean
  readonly electronPlatformWriterAdapterPreflightStatus?: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult['status']
  readonly electronPlatformWriterHostStatus?: ThirdPartyDataPackElectronPlatformWriterHostConnectionStatus
  readonly requestedCommandId?: 'install'
  readonly targetPackageId?: PackageId
  readonly selectedPackageIds: readonly PackageId[]
  readonly blockedPackageIds: readonly PackageId[]
  ...
  readonly writeProbeEvidence: ThirdPartyDataPackInstallTransactionWriteProbeEvidence
  readonly electronRequirementIds: readonly string[]
  readonly diagnostics: readonly ThirdPartyDataPackElectronPlatformWriterHostConnectionSafeDiagnostic[]
  readonly effects: ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceEffectSummary
}

export interface CreateThirdPartyDataPackElectronPlatformWriterHostConnectionSourceOptions {
  readonly enabled?: boolean
  readonly readElectronPlatformWriterAdapterPreflight?: () =>
    Awaitable<ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult>
  readonly connectElectronPlatformWriterHost?: (
    envelope: ThirdPartyDataPackElectronPlatformWriterHostConnectionEnvelope
  ) => Awaitable<ThirdPartyDataPackElectronPlatformWriterHostConnectionResult>
}

export class ThirdPartyDataPackElectronPlatformWriterHostConnectionBlockedError extends Error {
  readonly result: ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult

  constructor(result: ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult) {
    super('third-party Electron platform writer host connection blocked command continuation')
    this.name = 'ThirdPartyDataPackElectronPlatformWriterHostConnectionBlockedError'
    this.result = result
  }
}
...
const flexibleAdapterEffectFields = new Set<string>([
  'electronPlatformWriterAdapterPreflightCalled',
  'platformWriterConnectionPreflightCalled',
  'upstreamPlatformWriterConnectionReady',
  'electronPlatformWriterAdapterReady',
...
const forbiddenHostFields = [
  'electronPlatformWriterAdapterPreflight',
  'electronPlatformWriterHost',
  'realElectronPlatformWriterHost',
  'electronWriterHost',
  'electronHost',
...
const cloneElectronRequirementIds = (value: unknown): string[] => {
  if (!Array.isArray(value)) return []
  const length = readArrayLength(value)
  if (length === undefined) return []
...
): ThirdPartyDataPackElectronPlatformWriterHostConnectionSafeDiagnostic[] => {
  if (!Array.isArray(value)) return []
  const length = readArrayLength(value)
  if (length === undefined) return []

  const output: ThirdPartyDataPackElectronPlatformWriterHostConnectionSafeDiagnostic[] = []
  for (let index = 0; index < length; index += 1) {
    const descriptor = Reflect.getOwnPropertyDescriptor(value, String(index))
    if (!descriptor || !('value' in descriptor)) return []
...
const pathFreeAdapterSource = (
  source: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
...
const safeReadySource = (
  ...
  return readOwnStringField(source, 'status') === 'ready'
    ...
    && readOwnStringField(source, 'electronConnectionRequirementStatus') === 'required'
    && cloneElectronRequirementIds(readOwnDataField(source, 'electronRequirements')).length > 0
...
const hostEffectsSafe = (
  result: ThirdPartyDataPackElectronPlatformWriterHostConnectionResult
): boolean => {
  const effects = readOwnDataField(result, 'effects')
  return readOwnBooleanField(effects, 'electronPlatformWriterHostCalled') === true
    && readOwnBooleanField(effects, 'electronPlatformWriterHostAccepted') === true
    && readOwnBooleanField(effects, 'electronPlatformWriterConnected') === true
    && readOwnBooleanField(effects, 'programDirectoryUserdataResolved') === true
    && readOwnBooleanField(effects, 'electronIpcExposed') === false
...
const hostMatchesEnvelope = (
  ...
  && arraysEqual(cloneStringList(readOwnDataField(result, 'electronRequirementIds')), envelope.electronRequirementIds)
  && hostEffectsSafe(result)

const buildHostEnvelope = (
  source: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
): ThirdPartyDataPackElectronPlatformWriterHostConnectionEnvelope => {
  ...
  return deepFreezeObjectGraph({
    ...
    electronRequirementIds: cloneElectronRequirementIds(readOwnDataField(source, 'electronRequirements'))
  })
}
...
const effectSummary = (
  options: {
    ...
    readonly source?: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
    readonly hostAccepted?: boolean
  }
): ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceEffectSummary => {
  const hostAccepted = options.hostAccepted === true
  const sourceEffects = readOwnDataField(options.source, 'effects')
  return Object.freeze({
    electronPlatformWriterHostConnectionSourceCalled: true,
    electronPlatformWriterAdapterPreflightCalled: options.sourceCalled,
    injectedElectronPlatformWriterHostCalled: hostAccepted,
    electronPlatformWriterHostCalled: hostAccepted,
    electronPlatformWriterHostAccepted: hostAccepted,
    realElectronPlatformWriterHostCalled: false,
    electronPlatformWriterConnected: hostAccepted,
    programDirectoryUserdataResolved: hostAccepted,
...
const baseResult = (
  options: {
    ...
    readonly sourceCalled: boolean
    readonly source?: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
    readonly hostResult?: ThirdPartyDataPackElectronPlatformWriterHostConnectionResult
    readonly diagnostics?: readonly ThirdPartyDataPackElectronPlatformWriterHostConnectionSafeDiagnostic[]
  }
): ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult => {
  ...
  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_KIND,
    mode: THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_HOST_CONNECTION_SOURCE_MODE,
    ...
    electronPlatformWriterAdapterPreflightStatus: readOwnStringField(options.source, 'status') as
      | ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult['status']
      | undefined,
    electronPlatformWriterHostStatus: readOwnStringField(options.hostResult, 'status') as
      | ThirdPartyDataPackElectronPlatformWriterHostConnectionStatus
      | undefined,
    ...
    electronRequirementIds: cloneElectronRequirementIds(readOwnDataField(options.source, 'electronRequirements')),
...
const evaluateElectronPlatformWriterHostConnectionSource = async(
  options: CreateThirdPartyDataPackElectronPlatformWriterHostConnectionSourceOptions
): Promise<ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult> => {
  if (options.enabled !== true) {
    return baseResult({
      status: 'skipped',
      reason: 'third-party Electron platform writer host connection source is disabled by default',
      enabled: false,
      sourceCalled: false
    })
  }

  if (options.readElectronPlatformWriterAdapterPreflight === undefined) {
    return baseResult({
      status: 'blocked',
      reason: 'third-party Electron platform writer host connection source is enabled without an adapter preflight',
      enabled: true,
      sourceCalled: false,
      diagnostics: [
        commandDiagnostic('third-party.electron-platform-writer-host-connection.missing-source')
      ]
    })
  }

  let source: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
  try {
    source = await options.readElectronPlatformWriterAdapterPreflight()
  } catch {
    return baseResult({
      status: 'blocked',
      reason: 'third-party Electron platform writer adapter preflight failed before host connection',
      enabled: true,
      sourceCalled: true,
      diagnostics: [
        commandDiagnostic('third-party.electron-platform-writer-host-connection.source-failed')
      ]
    })
  }
  ...
  if (safeSkippedSource(source)) {
    return baseResult({
      status: 'skipped',
      reason: 'third-party Electron platform writer host connection is not required because adapter preflight was skipped',
      enabled: true,
      sourceCalled: true,
      source,
  ...
  if (!safeReadySource(source)) {
    ...
    return baseResult({
      status: 'blocked',
      reason: 'third-party Electron platform writer host connection requires safe ready Electron adapter preflight',
      enabled: true,
      sourceCalled: true,
      source,
      diagnostics: [
        ...sourceDiagnostics,
        commandDiagnostic('third-party.electron-platform-writer-host-connection.unsafe-source', targetPackageId)
      ]
    })
  }

  if (options.connectElectronPlatformWriterHost === undefined) {
    return baseResult({
      status: 'blocked',
      reason: 'third-party Electron platform writer host connection is enabled without an injected host',
      enabled: true,
      sourceCalled: true,
      source,
      diagnostics: [
        ...sourceDiagnostics,
        commandDiagnostic(
          'third-party.electron-platform-writer-host-connection.missing-host',
  ...
  let hostResult: ThirdPartyDataPackElectronPlatformWriterHostConnectionResult
  const envelope = buildHostEnvelope(source)
  try {
    hostResult = await options.connectElectronPlatformWriterHost(envelope)
  } catch {
    return baseResult({
      status: 'blocked',
      reason: 'third-party Electron platform writer host failed before returning a safe connection result',
      enabled: true,
      sourceCalled: true,
      source,
      diagnostics: [
        ...sourceDiagnostics,
        commandDiagnostic('third-party.electron-platform-writer-host-connection.host-failed', envelope.targetPackageId)
      ]
    })
  }
  ...
  if (hostMatchesEnvelope(envelope, hostResult)) {
    return baseResult({
      status: 'connected',
      reason: 'third-party Electron platform writer host connection accepted a path-free host acknowledgement',
      enabled: true,
      sourceCalled: true,
      source,
      hostResult,
  ...
  return baseResult({
    status: 'blocked',
    reason: 'third-party Electron platform writer host returned an unsafe or mismatched connection result',
    enabled: true,
    sourceCalled: true,
    source,
    hostResult,
    diagnostics: [
      ...sourceDiagnostics,
      ...hostDiagnostics,
      commandDiagnostic('third-party.electron-platform-writer-host-connection.unsafe-host', envelope.targetPackageId)
    ]
  })
}

export const createThirdPartyDataPackElectronPlatformWriterHostConnectionSource = (
  options: CreateThirdPartyDataPackElectronPlatformWriterHostConnectionSourceOptions = {}
): (() => Promise<ThirdPartyDataPackElectronPlatformWriterHostConnectionSourceResult>) => async() => {
  const result = await evaluateElectronPlatformWriterHostConnectionSource(options)
  if (result.status === 'blocked') {
    throw new ThirdPartyDataPackElectronPlatformWriterHostConnectionBlockedError(result)
  }
  return result
}

export const thirdPartyDataPackElectronPlatformWriterHostConnectionSource =
