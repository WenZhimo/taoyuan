...
export const THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_KIND =
  'web-startup-gate-decision-envelope'
export const THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_MODE =
  'readonly-launcher-boundary-decision'

export type ThirdPartyDataPackWebStartupGateDecisionEnvelopeStatus =
  | 'ready'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackWebStartupGateDecision =
  | 'ready-for-launcher-boundary'
  | 'not-required'
  | 'blocked-before-launcher-boundary'

export type ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheckId =
  | 'web-startup-gate-execution-wrapper'
...
export interface ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck {
  readonly id: ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheckId
  readonly status: 'satisfied' | 'skipped' | 'blocked'
  readonly reason: string
}

export interface ThirdPartyDataPackWebStartupGatePersistentStateProofs {
...
export interface ThirdPartyDataPackWebStartupGateDecisionEnvelopeEffectSummary {
  ...
  readonly webStartupGateExecutionConsumed: boolean
  readonly startupDecisionEnvelopeBuilt: boolean
}

export interface ThirdPartyDataPackWebStartupGateDecisionEnvelope {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_MODE
  readonly platform: 'web'
  readonly status: ThirdPartyDataPackWebStartupGateDecisionEnvelopeStatus
  readonly startupGateDecision: ThirdPartyDataPackWebStartupGateDecision
  readonly reason: string
  ...
  readonly executionStatus?: ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult['startupGateHandoffStatus']
  ...
  readonly persistentStateProofs?: ThirdPartyDataPackWebStartupGatePersistentStateProofs
  readonly checks: readonly ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck[]
  ...
  readonly effects: ThirdPartyDataPackWebStartupGateDecisionEnvelopeEffectSummary
}

export interface BuildThirdPartyDataPackWebStartupGateDecisionEnvelopeOptions {
  readonly execution: ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult
}
...
const safeDiagnostic = (
  ...
  return Object.freeze({
    ...
    stage: readOwnStringField(diagnostic, 'stage')
      ?? 'third-party.web-startup-gate-decision-envelope.diagnostic-copy',
...
const check = (
  id: ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheckId,
  status: ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck['status'],
  reason: string
): ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck => Object.freeze({ id, status, reason })

const skippedChecks = (
  reason: string
): readonly ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck[] => Object.freeze([
  'web-startup-gate-execution-wrapper',
  'startup-handoff-deferred',
  'persistent-state-preflight-deferred',
  'source-adapter-executed',
  ...
  id as ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheckId,
  'skipped',
  reason
)))
...
const adapterResultFromExecution = (
  execution: ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult
...
const executionWrapperReady = (
  ...
  && readOwnStringField(execution, 'mode')
    === THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_PERSISTENT_STATE_EXECUTION_MODE
  && readOwnStringField(execution, 'platform') === 'web'
...
): ThirdPartyDataPackWebStartupGatePersistentStateProofs | undefined => {
...
const proofsReady = (
  proofs: ThirdPartyDataPackWebStartupGatePersistentStateProofs | undefined
...
const buildChecks = (
  ...
  check(
    ...
    'Decision envelope must consume the narrow Web startup gate persistent-state execution wrapper.'
  ),
  check(
    'startup-handoff-deferred',
  ...
  check(
    ...
    readOwnStringField(adapterResult, 'status') === 'executed'
      ...
      ? 'satisfied'
      : 'blocked',
    'Web startup source adapter execution must have normalized a real persistent-state snapshot.'
  ),
  check(
    'startup-state-snapshot-normalized',
    snapshot !== undefined
...
const blockedDiagnostics = (
  checks: readonly ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck[],
  packageId?: PackageId
  ...
  .map(currentCheck => commandDiagnostic(
    `third-party.web-startup-gate-decision-envelope.checks.${currentCheck.id}`,
    packageId
  )))

const createEffectSummary = (
  executionConsumed: boolean
): ThirdPartyDataPackWebStartupGateDecisionEnvelopeEffectSummary => ({
  ...
  webStartupGateExecutionConsumed: executionConsumed,
...
const baseEnvelope = (
  status: ThirdPartyDataPackWebStartupGateDecisionEnvelopeStatus,
  startupGateDecision: ThirdPartyDataPackWebStartupGateDecision,
  reason: string,
  execution: ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult,
  adapterResult: ThirdPartyDataPackStartupGatePersistentStateSourceAdapterResult | undefined,
  checks: readonly ThirdPartyDataPackWebStartupGateDecisionEnvelopeCheck[],
  ...
  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_KIND,
    mode: THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_MODE,
    platform: 'web',
    status,
    startupGateDecision,
    reason,
    ...
    executionStatus: readOwnStringField(execution, 'startupGateHandoffStatus') as ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult['startupGateHandoffStatus'] | undefined,
...
export const buildThirdPartyDataPackWebStartupGateDecisionEnvelope = (
  options: BuildThirdPartyDataPackWebStartupGateDecisionEnvelopeOptions
): ThirdPartyDataPackWebStartupGateDecisionEnvelope => {
  ...
  if (
    ...
    return baseEnvelope(
      ...
      'blocked-before-launcher-boundary',
      extraDiagnostics.length > 0
        ? 'web startup gate decision envelope inputs are inconsistent'
        : readOwnStringField(adapterResult, 'reason') ?? 'web startup gate decision envelope is blocked upstream',
      execution,
      adapterResult,
      checks,
      diagnostics,
  ...
  return baseEnvelope(
    'ready',
    'ready-for-launcher-boundary',
    'web startup gate persistent state is ready for a later LauncherApp/GameApp boundary decision; app creation remains deferred',
    execution,
    adapterResult,
    checks,
    diagnostics,
