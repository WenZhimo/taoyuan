...
export const ANDROID_FILE_PICKER_IMPORT_SOURCE_KIND = 'android-file-picker-import'
export const ANDROID_FILE_PICKER_IMPORT_SOURCE_ID = 'android/file-picker-import'
export const ANDROID_FILE_PICKER_IMPORT_ROOT_PATH = 'android-import'

export interface AndroidFilePickerImportedFile {
  readonly relativePath: string
  readonly sizeBytes?: number
  readText(): Promise<string>
}

export interface CreateAndroidFilePickerImportSourceOptions {
  readonly files: readonly AndroidFilePickerImportedFile[]
  readonly sourceId?: string
  readonly rootPath?: string
  readonly policy?: ContentPackageSourceSafeReadPolicy
}

interface NormalizedAndroidFilePickerImportedFile {
  readonly path: string
  readonly file: AndroidFilePickerImportedFile
...
const asOptionsObject = (options: unknown): Record<string, unknown> => {
  if (typeof options !== 'object' || options === null || Array.isArray(options)) {
    throw new ContentPackageSourceError(
      ...
      'Android file-picker import source options metadata must be an object'
    )
  }
...
const readOptionsKeys = (options: Record<string, unknown>): readonly string[] => {
  ...
  try {
    ...
    throw new ContentPackageSourceError(
      'SOURCE_IDENTITY_INVALID',
      'Android file-picker import source options metadata could not be read'
    )
  }
  ...
  for (const key of keys) {
    if (typeof key !== 'string' || !supportedOptionsKeys.has(key)) {
      throw new ContentPackageSourceError(
        'SOURCE_IDENTITY_INVALID',
        'Android file-picker import source options metadata contains unsupported fields'
      )
    }
  ...
  if (requiredOptionsKeys.some(key => !ownKeys.has(key))) {
    throw new ContentPackageSourceError(
      'SOURCE_IDENTITY_INVALID',
      'Android file-picker import source options metadata must include files own field'
    )
  }
  return stringKeys
}
...
const toAndroidFileMetadataError = (): ContentPackageSourceError =>
  new ContentPackageSourceError(
    'SOURCE_ENTRY_UNSAFE',
    'Android file-picker import file metadata could not be read'
  )

const readAndroidFileRelativePath = (
  file: AndroidFilePickerImportedFile
): string => {
  try {
    const value = file.relativePath
    if (typeof value === 'string') return value
  } catch {
    throw toAndroidFileMetadataError()
  }
  throw new ContentPackageSourceError(
    'SOURCE_ENTRY_UNSAFE',
    'Android file-picker import file relativePath metadata must be a string'
  )
}

const readAndroidFileSize = (
  file: AndroidFilePickerImportedFile,
  sourcePath: string
): number | undefined => {
  let value: unknown
...
const readAndroidFileTextMethod = (
  file: AndroidFilePickerImportedFile
): (() => Promise<string>) => {
  let textMethod: unknown
  try {
    textMethod = file.readText
  } catch {
    throw toAndroidFileMetadataError()
  }
  if (typeof textMethod !== 'function') {
    throw new ContentPackageSourceError(
      'SOURCE_ENTRY_UNSAFE',
      'Android file-picker import file text reader must be a function'
    )
  }
  return () => Promise.resolve(textMethod.call(file) as Promise<string>)
}

const androidUriLikePathPattern = /^[A-Za-z][A-Za-z0-9+.-]*:\/\//
const windowsDrivePathPattern = /^[A-Za-z]:[\\/]/

const normalizeAndroidFilePath = (
  pathInput: string,
  ...
  if (
    ...
    || androidUriLikePathPattern.test(pathInput)
    || windowsDrivePathPattern.test(pathInput)
  ) {
    throw new ContentPackageSourceError(
      'SOURCE_PATH_UNSAFE',
...
const normalizeAndroidFile = (
  file: AndroidFilePickerImportedFile,
  policy: ContentPackageSourceSafeReadPolicy
): NormalizedAndroidFilePickerImportedFile => {
  if (typeof file !== 'object' || file === null) {
    throw new ContentPackageSourceError(
      'SOURCE_ENTRY_UNSAFE',
      'Android file-picker import file metadata must be an object'
    )
  }
  const path = normalizeAndroidFilePath(readAndroidFileRelativePath(file), policy)
  const readText = readAndroidFileTextMethod(file)
...
const readFiles = (
  files: unknown,
  policy: ContentPackageSourceSafeReadPolicy
): readonly NormalizedAndroidFilePickerImportedFile[] => {
  if (!Array.isArray(files)) {
    throw new ContentPackageSourceError(
      'SOURCE_ENTRY_UNSAFE',
      'Android file-picker import files metadata must be an array'
    )
  }
  if (files.length > policy.maxPackageFileCount) {
    throw new ContentPackageSourceError(
      'SOURCE_LIMIT_EXCEEDED',
      `Android file-picker import exceeds ${policy.maxPackageFileCount} files: ${files.length}`
    )
  }
  return files.map(file => normalizeAndroidFile(file as AndroidFilePickerImportedFile, policy))
}
...
const createSourceIdentity = (
  ...
  kind: ANDROID_FILE_PICKER_IMPORT_SOURCE_KIND,
  sourceId,
  rootPath
})

export const createAndroidFilePickerImportSource = (
  options: CreateAndroidFilePickerImportSourceOptions
  ...
  const sourceId = normalizeIdentityPart(
    optionKeys.includes('sourceId')
      ...
      : ANDROID_FILE_PICKER_IMPORT_SOURCE_ID,
    'sourceId'
  )
  ...
  const fileReaders = new Map<string, NormalizedAndroidFilePickerImportedFile>()
  ...
  const assertAvailable = (): void => {
    if (disposed) {
      throw new ContentPackageSourceError(
        'SOURCE_DISPOSED',
        'Android file-picker import source has been disposed'
      )
    }
  }
  ...
  return {
    ...
    async readTextFile(sourcePath) {
      ...
      const size = readAndroidFileSize(file.file, normalizedPath)
