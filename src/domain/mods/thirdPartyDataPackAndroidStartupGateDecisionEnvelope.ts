...
} from './thirdPartyDataPackAndroidStartupGatePersistentStateExecution'
import type {
...
export const THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_KIND =
  'android-startup-gate-decision-envelope'
export const THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_MODE =
  'readonly-launcher-boundary-decision'

export type ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeStatus =
  | 'ready'
  | 'skipped'
  | 'blocked'

export type ThirdPartyDataPackAndroidStartupGateDecision =
  | 'ready-for-launcher-boundary'
  | 'not-required'
  | 'blocked-before-launcher-boundary'

export type ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheckId =
  | 'android-startup-gate-execution-wrapper'
...
export interface ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck {
  readonly id: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheckId
  readonly status: 'satisfied' | 'skipped' | 'blocked'
  readonly reason: string
}

export interface ThirdPartyDataPackAndroidStartupGatePersistentStateProofs {
...
export interface ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeEffectSummary {
  ...
  readonly androidStartupGateExecutionConsumed: boolean
  readonly startupDecisionEnvelopeBuilt: boolean
}

export interface ThirdPartyDataPackAndroidStartupGateDecisionEnvelope {
  readonly kind: typeof THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_KIND
  readonly mode: typeof THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_MODE
  readonly platform: 'android'
  readonly status: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeStatus
  readonly startupGateDecision: ThirdPartyDataPackAndroidStartupGateDecision
  readonly reason: string
  ...
  readonly executionStatus?: ThirdPartyDataPackAndroidStartupGatePersistentStateExecutionResult['startupGateHandoffStatus']
  ...
  readonly persistentStateProofs?: ThirdPartyDataPackAndroidStartupGatePersistentStateProofs
  readonly checks: readonly ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck[]
  ...
  readonly effects: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeEffectSummary
}

export interface BuildThirdPartyDataPackAndroidStartupGateDecisionEnvelopeOptions {
  readonly execution: ThirdPartyDataPackAndroidStartupGatePersistentStateExecutionResult
}
...
const safeDiagnostic = (
  ...
  return Object.freeze({
    ...
    stage: readOwnStringField(diagnostic, 'stage')
      ?? 'third-party.android-startup-gate-decision-envelope.diagnostic-copy',
...
const check = (
  id: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheckId,
  status: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck['status'],
  reason: string
): ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck => Object.freeze({ id, status, reason })

const skippedChecks = (
  reason: string
): readonly ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck[] => Object.freeze([
  'android-startup-gate-execution-wrapper',
  'startup-handoff-deferred',
  'persistent-state-preflight-deferred',
  'source-adapter-executed',
  ...
  id as ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheckId,
  'skipped',
  reason
)))
...
const adapterResultFromExecution = (
  execution: ThirdPartyDataPackAndroidStartupGatePersistentStateExecutionResult
...
const executionWrapperReady = (
  ...
  && readOwnStringField(execution, 'mode')
    === THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_PERSISTENT_STATE_EXECUTION_MODE
  && readOwnStringField(execution, 'platform') === 'android'
...
): ThirdPartyDataPackAndroidStartupGatePersistentStateProofs | undefined => {
...
const proofsReady = (
  proofs: ThirdPartyDataPackAndroidStartupGatePersistentStateProofs | undefined
...
const buildChecks = (
  ...
  check(
    ...
    'Decision envelope must consume the narrow Android startup gate persistent-state execution wrapper.'
  ),
  check(
    'startup-handoff-deferred',
  ...
  check(
    ...
    readOwnStringField(adapterResult, 'status') === 'executed'
      ...
      ? 'satisfied'
      : 'blocked',
    'Android startup source adapter execution must have normalized a real persistent-state snapshot.'
  ),
  check(
    'startup-state-snapshot-normalized',
    snapshot !== undefined
...
const blockedDiagnostics = (
  checks: readonly ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck[],
  packageId?: PackageId
  ...
  .map(currentCheck => commandDiagnostic(
    `third-party.android-startup-gate-decision-envelope.checks.${currentCheck.id}`,
    packageId
  )))

const createEffectSummary = (
  executionConsumed: boolean
): ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeEffectSummary => ({
  ...
  androidStartupGateExecutionConsumed: executionConsumed,
...
const baseEnvelope = (
  status: ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeStatus,
  startupGateDecision: ThirdPartyDataPackAndroidStartupGateDecision,
  reason: string,
  execution: ThirdPartyDataPackAndroidStartupGatePersistentStateExecutionResult,
  adapterResult: ThirdPartyDataPackStartupGatePersistentStateSourceAdapterResult | undefined,
  checks: readonly ThirdPartyDataPackAndroidStartupGateDecisionEnvelopeCheck[],
  ...
  return deepFreezeObjectGraph({
    kind: THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_KIND,
    mode: THIRD_PARTY_DATA_PACK_ANDROID_STARTUP_GATE_DECISION_ENVELOPE_MODE,
    platform: 'android',
    status,
    startupGateDecision,
    reason,
    ...
    executionStatus: readOwnStringField(execution, 'startupGateHandoffStatus') as ThirdPartyDataPackAndroidStartupGatePersistentStateExecutionResult['startupGateHandoffStatus'] | undefined,
...
export const buildThirdPartyDataPackAndroidStartupGateDecisionEnvelope = (
  options: BuildThirdPartyDataPackAndroidStartupGateDecisionEnvelopeOptions
): ThirdPartyDataPackAndroidStartupGateDecisionEnvelope => {
  ...
  if (
    ...
    return baseEnvelope(
      ...
      'blocked-before-launcher-boundary',
      extraDiagnostics.length > 0
        ? 'android startup gate decision envelope inputs are inconsistent'
        : readOwnStringField(adapterResult, 'reason') ?? 'android startup gate decision envelope is blocked upstream',
      execution,
      adapterResult,
      checks,
      diagnostics,
  ...
  return baseEnvelope(
    'ready',
    'ready-for-launcher-boundary',
    'android startup gate persistent state is ready for a later LauncherApp/GameApp boundary decision; app creation remains deferred',
    execution,
    adapterResult,
    checks,
    diagnostics,
