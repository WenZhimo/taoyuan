...
} from '@/domain/mods/thirdPartyDataPackAndroidUiIpcResponseDeliveryPipeline'
import {
...
const expectNoRuntimeStartupOrWriteEffects = (
  ...
  expect(result.effects.androidResponseDeliveryAcknowledgementConsumed).toBe(delivered)
...
const createAndroidHost = (): ThirdPartyDataPackAndroidResponseDeliverySinkHost & {
  ...
  deliver: vi.fn(async envelope => {
    ...
    expect('androidHost' in envelope).toBe(false)
    expect('androidNativeBridge' in envelope).toBe(false)
    expect('capacitorBridge' in envelope).toBe(false)
    return {
      status: 'acknowledged',
      channel: 'android-native-response-event-sink',
    ...
    } satisfies ThirdPartyDataPackAndroidResponseDeliveryAcknowledgement
  })
})

describe('third-party Android UI/IPC response delivery pipeline', () => {
  it('is disabled by default and does not read upstream handoffs or call the Android host', async() => {
    ...
    const pipeline = createThirdPartyDataPackAndroidUiIpcResponseDeliveryPipeline({
  ...
  it('delivers an Android response and converges the acknowledgement without touching other platforms', async() => {
    ...
    expect('androidNativeBridge' in result).toBe(false)
  ...
  it('blocks before the Android host when orchestration inputs drift', async() => {
    const readResultNormalizationPreflight = vi.fn(async() => createResultNormalizationPreflight({
