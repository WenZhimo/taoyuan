...
import {
  ...
  THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_SINK_SOURCE_KIND,
  THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_SINK_SOURCE_MODE,
  ThirdPartyDataPackElectronResponseDeliverySinkBlockedError,
  type ThirdPartyDataPackElectronResponseDeliverySinkSourceResult
} from '@/domain/mods/thirdPartyDataPackElectronResponseDeliverySinkSource'
...
const expectNoRuntimeOrPersistentEffects = (
  result: ThirdPartyDataPackElectronResponseDeliverySinkSourceResult,
  delivered: boolean
): void => {
  expect(result.uiIpcResponseDeliveryAllowed).toBe(delivered)
  expect(result.electronIpcAllowed).toBe(delivered)
  expect(result.electronResponseDeliveryAllowed).toBe(delivered)
  ...
  if (delivered) expect(result.effects.electronSinkHostCalled).toBe(true)
  expect(result.effects.deliveryAcknowledgementReceived).toBe(delivered)
  expect(result.effects.electronResponseDelivered).toBe(delivered)
...
describe('third-party Electron response delivery sink source', () => {
  it('is disabled by default and does not call platform split or Electron sink host sources', async() => {
    ...
    const source = createThirdPartyDataPackElectronResponseDeliverySinkSource({
    ...
    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_SINK_SOURCE_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_ELECTRON_RESPONSE_DELIVERY_SINK_SOURCE_MODE)
    expect(result.platform).toBe('electron')
    expect(result.status).toBe('skipped')
    expect(result.enabled).toBe(false)
    expect(result.sourceCalled).toBe(false)
    expect(result.electronSinkAdapterCalled).toBe(false)
  ...
  it('delivers through the injected fixed Electron sink host when explicitly enabled', async() => {
    ...
    expect(result.electronSinkAdapterCalled).toBe(true)
    ...
    expect(result.electronResponseDeliverySinkAdapterStatus).toBe('delivered')
  ...
  it('allows continuation when an enabled platform split source reports skipped', async() => {
    ...
    expect(result.electronResponseDeliverySinkAdapterStatus).toBe('skipped')
    expect(host.deliver).not.toHaveBeenCalled()
  ...
  it('blocks missing sources, host failures and invalid acknowledgements without leaking host paths', async() => {
    const missingSource = createThirdPartyDataPackElectronResponseDeliverySinkSource({
      enabled: true,
      host: {
    ...
    const throwingSource = createThirdPartyDataPackElectronResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => {
        throw new Error('EACCES: C:/Users/LENOVO/mods/electron-source-platform-split')
      },
      host: {
        channel: 'electron-preload-response-channel',
        deliver: envelope => createAcknowledgement(envelope)
    ...
    const hostFailureSource = createThirdPartyDataPackElectronResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => createPlatformSplitContract(),
      host: {
        channel: 'electron-preload-response-channel',
        deliver: () => {
          throw new Error('EACCES C:/Users/LENOVO/mods/electron-source-host')
        }
      }
    })
    const invalidAcknowledgementSource = createThirdPartyDataPackElectronResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => createPlatformSplitContract(),
      host: {
        channel: 'electron-preload-response-channel',
    ...
    await expect(missingSource()).rejects.toBeInstanceOf(ThirdPartyDataPackElectronResponseDeliverySinkBlockedError)

    for (const currentSource of [throwingSource, hostFailureSource, invalidAcknowledgementSource]) {
      try {
        await currentSource()
      } catch (error) {
        expect(error).toBeInstanceOf(ThirdPartyDataPackElectronResponseDeliverySinkBlockedError)
        const result = (error as ThirdPartyDataPackElectronResponseDeliverySinkBlockedError).result
  ...
  it('blocks unsafe platform split drift while copying hostile arrays safely', async() => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/electron-source-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/electron-source-diagnostic-prototype')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.electron-response-delivery-sink-source.proxy-test',
    ...
    try {
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({
          stage: 'third-party.electron-response-delivery-sink-source.delivery-blocked'
        })
      ]))
      const serialized = JSON.stringify(result)
      expect(serialized).not.toContain('C:/Users')
      expect(serialized).not.toContain('LENOVO')
      expect(serialized).not.toContain('electron-source-selected-package-ids')
