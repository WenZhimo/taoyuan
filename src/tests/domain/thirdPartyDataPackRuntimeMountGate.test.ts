...
describe('third-party data pack runtime mount gate', () => {
  ...
  it('ignores own accessor mount input diagnostic fields before exposing runtime mount gate reports', () => {
    const packageId = requirePackageId('discovery_valid')
    const relatedPackageId = requirePackageId('related_pack')
    const upstreamDiagnostic = createDiagnostic('CACHE-INVALID-001', {
      stage: 'third-party.runtime-mount.accessor-test',
      severity: 'warning',
      packageId,
      relatedPackageIds: [relatedPackageId],
    ...
    Object.defineProperty(upstreamDiagnostic, 'stage', {
      ...
      get() {
        stageRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-diagnostic-stage')
      }
    })
    Object.defineProperty(upstreamDiagnostic, 'relatedPackageIds', {
      ...
      get() {
        ...
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-diagnostic-related')
      }
    })
    Object.defineProperty(upstreamDiagnostic, 'details', {
      ...
      get() {
        detailsRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-diagnostic-details')
      }
    })
    const mountInput: ThirdPartyDataPackMountInputResult = {
      status: 'ready',
    ...
    if (!copiedDiagnostic) throw new Error('Expected copied runtime mount diagnostic.')
    ...
    expect(copiedDiagnostic).toMatchObject({
      ...
      stage: 'third-party.runtime-mount.diagnostic-copy',
    ...
    expect(serialized).not.toContain('runtime-mount-diagnostic-stage')
    expect(serialized).not.toContain('runtime-mount-diagnostic-related')
    expect(serialized).not.toContain('runtime-mount-diagnostic-details')
    expectNoWriteEffects(gate)
    expectOfficialBaseline()
  })

  it('copies mount input diagnostic array details without reading hostile proxy lengths', () => {
    const packageId = requirePackageId('discovery_valid')
    ...
    const listDetails = new Proxy(['first', { stable: true }] as JsonValue[], {
      get(target, property, receiver) {
        if (property === 'length') {
          lengthRead = true
          throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-list-length')
        }
        return Reflect.get(target, property, receiver)
      }
    })
    const upstreamDiagnostic = createDiagnostic('CACHE-INVALID-001', {
      stage: 'test.runtime-mount.proxy-array',
      severity: 'warning',
      packageId,
    ...
    if (copiedDiagnostic === undefined) throw new Error('Expected copied runtime mount diagnostic.')
    ...
    expect(JSON.stringify(gate)).not.toContain('C:/Users')
    expect(JSON.stringify(gate)).not.toContain('LENOVO')
    expect(JSON.stringify(gate)).not.toContain('runtime-mount-list-length')
    expectNoWriteEffects(gate)
    expectOfficialBaseline()
  })

  it('copies mount input diagnostic related package ids without reading hostile proxy lengths', () => {
    const packageId = requirePackageId('discovery_valid')
    const relatedPackageId = requirePackageId('related_pack')
    let lengthRead = false
    const relatedPackageIds = new Proxy([packageId, relatedPackageId], {
      get(target, property, receiver) {
        if (property === 'length') {
          lengthRead = true
          throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-related-package-length')
        }
        return Reflect.get(target, property, receiver)
      }
    }) as ReturnType<typeof requirePackageId>[]
    const upstreamDiagnostic = createDiagnostic('CACHE-INVALID-001', {
      stage: 'test.runtime-mount.proxy-related-package-ids',
      severity: 'warning',
      packageId,
      relatedPackageIds,
      details: {
    ...
    expect(JSON.stringify(gate)).not.toContain('C:/Users')
    expect(JSON.stringify(gate)).not.toContain('LENOVO')
    expect(JSON.stringify(gate)).not.toContain('runtime-mount-related-package-length')
    expectNoWriteEffects(gate)
    expectOfficialBaseline()
  })

  it('copies mount input diagnostics without reading hostile proxy lengths', () => {
    const packageId = requirePackageId('discovery_valid')
    let lengthRead = false
    const upstreamDiagnostic = createDiagnostic('CACHE-INVALID-001', {
      stage: 'test.runtime-mount.proxy-diagnostics',
      severity: 'warning',
      packageId,
      relatedPackageIds: [packageId],
      details: {
    ...
    const diagnostics = new Proxy([upstreamDiagnostic], {
      get(target, property, receiver) {
        if (property === 'length') {
          lengthRead = true
          throw new Error('EACCES: stat C:/Users/LENOVO/mods/runtime-mount-diagnostics-length')
        }
        return Reflect.get(target, property, receiver)
      }
    }) as ThirdPartyDataPackMountInputResult['diagnostics']
    const mountInput: ThirdPartyDataPackMountInputResult = {
      status: 'ready',
      preflightStatus: 'ready',
      reason: 'mount input is ready',
    ...
    expect(JSON.stringify(gate)).not.toContain('C:/Users')
    expect(JSON.stringify(gate)).not.toContain('LENOVO')
    expect(JSON.stringify(gate)).not.toContain('runtime-mount-diagnostics-length')
    expectNoWriteEffects(gate)
    expectOfficialBaseline()
  })

  it('copies mount input package summaries without reading hostile proxy lengths', () => {
    const packageId = requirePackageId('discovery_valid')
    ...
    const createHostileStringArray = <T extends string>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/runtime-mount-${label}-length`)
          }
          return Reflect.get(target, property, receiver)
        }
      }) as T[]
    ...
    expect(JSON.stringify(gate)).not.toContain('C:/Users')
    expect(JSON.stringify(gate)).not.toContain('LENOVO')
    expect(JSON.stringify(gate)).not.toContain('runtime-mount-selected-package-ids-length')
    expectNoWriteEffects(gate)
    expectOfficialBaseline()
  })
})
