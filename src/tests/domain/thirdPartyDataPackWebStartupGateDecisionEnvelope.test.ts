...
import {
  ...
  THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_KIND,
  THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_MODE,
  type ThirdPartyDataPackWebStartupGateDecisionEnvelope
...
const createExecutionResult = (
  ...
  overrides: Partial<ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult> = {}
): ThirdPartyDataPackWebStartupGatePersistentStateExecutionResult => ({
...
const expectNoRealStartupEffects = (
  result: ThirdPartyDataPackWebStartupGateDecisionEnvelope
): void => {
  ...
  const {
    webStartupGateExecutionConsumed: _webStartupGateExecutionConsumed,
...
describe('third-party Web startup gate decision envelope', () => {
  it('builds a path-free ready envelope while leaving LauncherApp and GameApp creation deferred', () => {
    const result = buildThirdPartyDataPackWebStartupGateDecisionEnvelope({
      execution: createExecutionResult()
    })

    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_WEB_STARTUP_GATE_DECISION_ENVELOPE_MODE)
    ...
    expect(result.messageKey).toBe('mods.startup.persistent.state.web.ready')
    ...
    expect(result.effects.webStartupGateExecutionConsumed).toBe(true)
    ...
    expect(JSON.stringify(result)).not.toContain('indexedDB://taoyuan/mod-imports')
  ...
  it('blocks upstream failures and strips inherited path-bearing diagnostic metadata', () => {
    const diagnostic = Object.assign(Object.create({
      get privatePath() {
        throw new Error('indexedDB://taoyuan/mod-imports/0/taoyuan-web.taoyuan/mods/startup-decision-diagnostic')
      }
    }), {
      code: 'LIFECYCLE-TRANSACTION-001',
      ruleId: 'LIFECYCLE-TRANSACTION-001',
      severity: 'error',
      stage: 'third-party.web-startup-gate-decision.blocked-source',
    ...
    expect(serialized).not.toContain('indexedDB://taoyuan/mod-imports')
    expect(serialized).not.toContain('taoyuan-web')
