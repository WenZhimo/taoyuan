...
import {
  ...
  THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_SOURCE_KIND,
  THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_SOURCE_MODE,
  ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffBlockedError,
  type ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSourceResult
} from '@/domain/mods/thirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource'
import type {
...
const createHandoffEffects = (
  prepared: boolean,
  overrides: Partial<ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffEffectSummary> = {}
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffEffectSummary => ({
...
const createHandoffResult = (
  overrides: Partial<ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult> = {}
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult => ({
  kind: 'android-response-delivery-startup-gate-handoff',
  mode: 'readonly-android-response-delivery-startup-gate-handoff',
  platform: 'android',
  status: 'ready',
  androidResponseDeliveryStatus: 'delivered',
  startupGateHandoffPreflightStatus: 'deferred',
  reason: 'android response delivery acknowledgement is connected to startup gate handoff evidence',
  androidResponseDeliveryStartupGateHandoff: 'ready',
  readOnly: true,
  androidResponseDelivered: true,
  ...
  checks: [
    {
      id: 'android-response-delivery-delivered',
      status: 'satisfied',
      reason: 'Android response delivery was delivered.'
    },
    {
      id: 'android-delivery-acknowledgement-present',
      status: 'satisfied',
      reason: 'Android acknowledgement was present.'
    },
    {
  ...
  ...overrides
} as unknown as ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult)

const createSkippedHandoffResult = (
  overrides: Partial<ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult> = {}
): ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult => createHandoffResult({
  status: 'skipped',
  androidResponseDeliveryStatus: 'skipped',
  startupGateHandoffPreflightStatus: 'skipped',
  reason: 'no selected third-party data packs',
  androidResponseDeliveryStartupGateHandoff: 'skipped',
  androidResponseDelivered: false,
...
const expectNoStartupRuntimeOrWriteEffects = (
  result: ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSourceResult,
  ...
  const {
    androidResponseDeliveryStartupGateHandoffSourceCalled: _sourceCalled,
    androidResponseDeliveryStartupGateHandoffReaderCalled: _readerCalled,
...
describe('third-party Android response delivery startup gate handoff source', () => {
  it('is disabled by default and does not call the handoff source', async() => {
    const readAndroidResponseDeliveryStartupGateHandoff = vi.fn()
    const source = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      readAndroidResponseDeliveryStartupGateHandoff
    })

    const result = await source()

    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_SOURCE_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_STARTUP_GATE_HANDOFF_SOURCE_MODE)
    expect(result.platform).toBe('android')
    expect(result.status).toBe('skipped')
    expect(result.enabled).toBe(false)
    expect(result.sourceCalled).toBe(false)
    expect(readAndroidResponseDeliveryStartupGateHandoff).not.toHaveBeenCalled()
  ...
  it('accepts a ready path-free Android response delivery startup handoff without exposing source internals', async() => {
    const readAndroidResponseDeliveryStartupGateHandoff = vi.fn(async() => createHandoffResult())
    const source = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      enabled: true,
      readAndroidResponseDeliveryStartupGateHandoff
    })
    ...
    expect(readAndroidResponseDeliveryStartupGateHandoff).toHaveBeenCalledOnce()
  ...
  it('allows skipped handoffs to continue without accepting Android delivery evidence', async() => {
    const source = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      enabled: true,
      readAndroidResponseDeliveryStartupGateHandoff: async() => createSkippedHandoffResult()
    })
    ...
    expect(result.androidResponseDelivered).toBe(false)
  ...
  it('blocks missing, throwing and blocked handoff sources without leaking host paths', async() => {
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/android-startup-handoff-diagnostic')
      }
    }), {
      code: 'LIFECYCLE-TRANSACTION-001',
      ruleId: 'LIFECYCLE-TRANSACTION-001',
      severity: 'error',
      stage: 'third-party.android-response-delivery-startup-gate-handoff-source.blocked-source',
      messageKey: 'mods.error.lifecycle.transaction.001',
      packageId,
      file: 'C:/Users/LENOVO/mods/sample_pack/startup-handoff.json',
      recovery: 'retry'
    })
    const missingSource = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      enabled: true
    })
    const throwingSource = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      enabled: true,
      readAndroidResponseDeliveryStartupGateHandoff: async() => {
        throw new Error('EACCES: C:/Users/LENOVO/mods/android-startup-handoff-source')
      }
    })
    const blockedSource = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      enabled: true,
      readAndroidResponseDeliveryStartupGateHandoff: async() => createHandoffResult({
        status: 'blocked',
        androidResponseDeliveryStartupGateHandoff: 'blocked',
        reason: 'upstream Android startup handoff blocked',
    ...
    try {
      ...
      expect(error).toBeInstanceOf(ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffBlockedError)
      const result = (error as ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffBlockedError).result
      expect(result.status).toBe('blocked')
      expect(result.sourceCalled).toBe(true)
      expect(result.diagnostics).toEqual([
        expect.objectContaining({
          stage: 'third-party.android-response-delivery-startup-gate-handoff-source.source-failed'
        })
      ])
    ...
    try {
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        ...
        expect.objectContaining({
          stage: 'third-party.android-response-delivery-startup-gate-handoff-source.handoff-blocked',
          packageId
        })
      ]))
  ...
  it('blocks unsafe handoff drift while copying package arrays without reading hostile lengths', async() => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/android-startup-handoff-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/android-startup-handoff-diagnostic-prototype')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.android-response-delivery-startup-gate-handoff-source.proxy-test',
    ...
    const source = createThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffSource({
      ...
      } as unknown as Partial<ThirdPartyDataPackAndroidResponseDeliveryStartupGateHandoffResult>)
    })
    ...
    try {
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({
          stage: 'third-party.android-response-delivery-startup-gate-handoff-source.unsafe-source'
        })
      ]))
      const serialized = JSON.stringify(result)
      expect(serialized).not.toContain('C:/Users')
      expect(serialized).not.toContain('LENOVO')
      expect(serialized).not.toContain('android-startup-handoff-selected-package-ids')
      expect(serialized).not.toContain('hostPath')
