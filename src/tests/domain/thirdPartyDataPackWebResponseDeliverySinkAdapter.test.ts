...
describe('third-party Web response delivery sink adapter', () => {
  it('delivers a cloned path-free envelope through an injected fixed Web sink host', async () => {
    ...
    expect(result.webResponseDeliverySinkAdapter).toBe('delivered')
    ...
    expect(result.electronIpcAllowed).toBe(false)
    expect(result.electronResponseDeliveryAllowed).toBe(false)
    expect(result.webUiBridgeAllowed).toBe(true)
    expect(result.webResponseDeliveryAllowed).toBe(true)
  ...
  it('skips or blocks upstream platform split states while stripping path-bearing diagnostics', async () => {
    ...
    const blockedSource = createPlatformSplitContract({
      status: 'blocked',
      reason: 'platform split blocked before Web delivery',
      resultEnvelopeContract: 'blocked',
      envelopeNormalized: false,
      diagnostics: [
        createDiagnostic('LIFECYCLE-TRANSACTION-001', {
          stage: 'third-party.web-response-delivery-sink.blocked-source',
          severity: 'error',
          packageId,
    ...
    const skipped = await deliverThirdPartyDataPackWebResponseDeliverySinkAdapter({
      platformSplitContract: skippedSource
    })
    const blocked = await deliverThirdPartyDataPackWebResponseDeliverySinkAdapter({
      platformSplitContract: blockedSource
    })

    expect(skipped.status).toBe('skipped')
    expect(skipped.webResponseDeliveryAttempted).toBe(false)
    expect(skipped.webResponseDelivered).toBe(false)
    expect(skipped.deliveryEnvelope).toBeUndefined()
    expect(blocked.status).toBe('blocked')
    expect(blocked.webResponseDeliveryAttempted).toBe(false)
  ...
  it('blocks missing host, wrong channel, missing Web contract, missing envelope and upstream effect drift', async () => {
    const source = createPlatformSplitContract()
    const cases = [
      {
        label: 'missing host',
        source,
        host: undefined,
        blockedCheckId: 'web-sink-host-ready'
      },
      {
        label: 'wrong channel',
        source,
        host: {
          channel: 'electron-preload-response-channel' as never,
          deliver: (envelope: ThirdPartyDataPackUiIpcResultEnvelope) => createAcknowledgement(envelope)
        },
        blockedCheckId: 'fixed-web-channel'
      },
      {
        label: 'missing Web platform adapter',
        source: {
          ...source,
          platformAdapters: source.platformAdapters.filter(adapter => adapter.platform !== 'web')
        },
        host: {
          channel: 'web-ui-response-event-sink' as const,
          deliver: (envelope: ThirdPartyDataPackUiIpcResultEnvelope) => createAcknowledgement(envelope)
        },
        blockedCheckId: 'web-platform-contract-present'
      },
      {
    ...
    for (const currentCase of cases) {
      ...
      expect(result.reason).toBe('Web response delivery sink inputs are inconsistent')
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({
          code: 'LIFECYCLE-TRANSACTION-001',
          stage: `third-party.web-response-delivery-sink.checks.${currentCase.blockedCheckId}`
        })
      ]))
  ...
  it('blocks host failures and invalid acknowledgements without leaking host paths', async () => {
    const source = createPlatformSplitContract()
    const hostFailure = await deliverThirdPartyDataPackWebResponseDeliverySinkAdapter({
      platformSplitContract: source,
      host: {
        channel: 'web-ui-response-event-sink',
        deliver: () => {
          throw new Error('EACCES C:/Users/LENOVO/mods/web-response-delivery')
        }
      }
    })
    const invalidAcknowledgement = await deliverThirdPartyDataPackWebResponseDeliverySinkAdapter({
      platformSplitContract: source,
      host: {
        channel: 'web-ui-response-event-sink',
        deliver: envelope => ({
    ...
    expect(hostFailure.reason).toBe('Web response delivery sink host failed before acknowledgement')
    expect(hostFailure.webResponseDeliveryAttempted).toBe(true)
    expect(hostFailure.webResponseDelivered).toBe(false)
    ...
    expect(invalidAcknowledgement.reason).toBe('Web response delivery sink host returned an invalid acknowledgement')
    expect(invalidAcknowledgement.webResponseDeliveryAttempted).toBe(true)
    expect(invalidAcknowledgement.webResponseDelivered).toBe(false)
  ...
  it('copies package summaries and diagnostics without reading hostile proxy lengths', async () => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/web-sink-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/web-sink-diagnostic-prototype')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.web-response-delivery-sink.proxy-test',
    ...
    expect(JSON.stringify(result)).not.toContain('web-sink-selected-package-ids')
  ...
  it('blocks accessor-based upstream effects without invoking path-bearing getters', async () => {
    ...
    Object.defineProperty(hostileEffects, 'webUiResponsePublished', {
      enumerable: true,
      get() {
        effectGetterRead = true
        throw new Error('C:/Users/LENOVO/mods/web-sink-effect-getter')
      }
    })
    const source = {
