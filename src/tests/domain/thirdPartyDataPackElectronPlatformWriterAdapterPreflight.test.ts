...
import {
  ...
  THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_ADAPTER_PREFLIGHT_KIND,
  THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_ADAPTER_PREFLIGHT_MODE,
  ThirdPartyDataPackElectronPlatformWriterAdapterPreflightBlockedError,
  type ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult
...
const expectNoRealElectronWriter = (
  result: ThirdPartyDataPackElectronPlatformWriterAdapterPreflightResult,
  ...
  expect(result.effects.programDirectoryUserdataResolved).toBe(false)
...
describe('third-party Electron platform writer adapter preflight', () => {
  it('is disabled by default and does not call the platform writer connection preflight', async() => {
    ...
    const preflight = createThirdPartyDataPackElectronPlatformWriterAdapterPreflight({
      readPlatformWriterConnectionPreflight
    })

    const result = await preflight()

    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_ADAPTER_PREFLIGHT_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_ELECTRON_PLATFORM_WRITER_ADAPTER_PREFLIGHT_MODE)
    ...
    expect(result.electronRequirements.every(requirement => requirement.status === 'skipped')).toBe(true)
    expectNoRealElectronWriter(result, false)
  ...
  it('allows continuation when an enabled platform writer connection preflight reports skipped', async() => {
    ...
    expect(result.electronConnectionRequirementStatus).toBe('skipped')
  ...
  it('accepts deferred Electron writer requirements while keeping real Electron wiring disabled', async() => {
    const preflight = createThirdPartyDataPackElectronPlatformWriterAdapterPreflight({
      enabled: true,
    ...
    expect(result.electronConnectionRequirementStatus).toBe('required')
    expect(result.targetPackageId).toBe(packageId)
    expect(result.selectedPackageIds).toEqual([packageId])
    ...
    expect(result.electronRequirements.map(requirement => ({
      id: requirement.id,
      status: requirement.status
    }))).toEqual([
      { id: 'electron-program-directory-userdata-root', status: 'required' },
      { id: 'electron-mod-lock-json-atomic-writer', status: 'required' },
      { id: 'electron-settings-json-atomic-writer', status: 'required' },
      { id: 'electron-path-free-preload-ipc', status: 'required' }
    ])
    expect(result.effects.upstreamPlatformWriterConnectionReady).toBe(true)
    expect(result.effects.electronPlatformWriterAdapterReady).toBe(true)
    expect('platformWriterConnectionPreflight' in result).toBe(false)
    expect('electronPlatformWriterHost' in result).toBe(false)
    expect('programDirectoryPath' in result).toBe(false)
    expectNoRealElectronWriter(result, true)
    expectJsonGraphFrozen(result)
  })

  it('blocks missing and throwing sources without leaking thrown details', async() => {
    const missingSource = createThirdPartyDataPackElectronPlatformWriterAdapterPreflight({
      enabled: true
    })
    const throwingSource = createThirdPartyDataPackElectronPlatformWriterAdapterPreflight({
      enabled: true,
      readPlatformWriterConnectionPreflight: async() => {
        throw new Error('EACCES: C:/Users/LENOVO/taoyuan/userdata/electron-writer')
      }
    })
    ...
    try {
      ...
      expect(error).toBeInstanceOf(ThirdPartyDataPackElectronPlatformWriterAdapterPreflightBlockedError)
      const result = (error as ThirdPartyDataPackElectronPlatformWriterAdapterPreflightBlockedError).result
      expect(result.status).toBe('blocked')
      expect(result.sourceCalled).toBe(true)
      expect(result.diagnostics).toEqual([
        expect.objectContaining({
          stage: 'third-party.electron-platform-writer-adapter-preflight.source-failed'
        })
      ])
      expect(JSON.stringify(result)).not.toContain('C:/Users')
      expect((error as Error).message).not.toContain('C:/Users')
      expectNoRealElectronWriter(result, false, false)
      expectJsonGraphFrozen(result)
    }
  })
  ...
  it('blocks unsafe path-bearing upstream connection preflights without exposing paths', async() => {
        ...
        diagnostics: [
          {
            ...
            severity: 'error',
            stage: 'third-party.electron-platform-writer-adapter-preflight.upstream-path',
    ...
    try {
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        ...
        expect.objectContaining({
          stage: 'third-party.electron-platform-writer-adapter-preflight.unsafe-source',
          packageId
        }),
        expect.objectContaining({
          stage: 'third-party.electron-platform-writer-adapter-preflight.connection-blocked',
          packageId
        })
      ]))
  ...
  it('blocks unsafe sources and copies package arrays without reading hostile lengths', async() => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/electron-writer-${label}`)
          }
    ...
    try {
      ...
      expect(serialized).not.toContain('C:/Users')
      expect(serialized).not.toContain('LENOVO')
      expect(serialized).not.toContain('electron-writer-selected-package-ids')
      expect('electronPlatformWriterHost' in result).toBe(false)
      expectNoRealElectronWriter(result, false, false)
      expectJsonGraphFrozen(result)
    }
