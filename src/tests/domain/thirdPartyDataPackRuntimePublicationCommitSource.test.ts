...
describe('third-party runtime publication commit source', () => {
  ...
  it('accepts a real in-memory runtime publication commit host without persistent writes', async() => {
    ...
    expect(result.status).toBe('accepted')
    expect(result.reason).toBe('runtime publication commit source accepted a real in-memory host acknowledgement')
    expect(result.runtimePublicationCommitHostStatus).toBe('accepted')
    expect(result.runtimePublicationCommitHostMode).toBe('real-in-memory-runtime-publication-commit-host')
    expect(result.injectedRuntimePublicationHostMode).toBeUndefined()
    expect(result.requiredCommitAdapterIds).toEqual([...requiredCommitAdapterIds])
    expect(result.effects.injectedRuntimePublicationCommitHostCalled).toBe(false)
    expect(result.effects.runtimePublicationCommitHostCalled).toBe(true)
    expect(result.effects.runtimePublicationCommitHostAccepted).toBe(true)
