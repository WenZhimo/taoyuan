...
const expectNoRuntimeStartupOrWriteEffects = (
  ...
  expect(result.effects.webResponseDeliveryAcknowledgementConsumed).toBe(delivered)
...
const createWebHost = (): ThirdPartyDataPackWebResponseDeliverySinkHost & {
  ...
  deliver: vi.fn(async envelope => {
    ...
    expect('webHost' in envelope).toBe(false)
    ...
    } satisfies ThirdPartyDataPackWebResponseDeliveryAcknowledgement
  })
})

describe('third-party Web UI/IPC response delivery pipeline', () => {
  it('is disabled by default and does not read upstream handoffs or call the Web host', async() => {
  ...
  it('delivers a Web response and converges the acknowledgement without touching other platforms', async() => {
  ...
  it('blocks before the Web host when orchestration inputs drift', async() => {
    const readResultNormalizationPreflight = vi.fn(async() => createResultNormalizationPreflight({
