...
describe('third-party Android response delivery sink adapter', () => {
  it('delivers a cloned path-free envelope through an injected fixed Android sink host', async () => {
    ...
    expect(result.androidResponseDeliverySinkAdapter).toBe('delivered')
    expect(result.sourcePlatformSplitStatus).toBe('deferred')
    expect(result.androidResponseDeliveryAttempted).toBe(true)
  ...
  it('skips or blocks upstream platform split states while stripping path-bearing diagnostics', async () => {
    ...
    const blockedSource = createPlatformSplitContract({
      status: 'blocked',
      reason: 'platform split blocked before Android delivery',
      resultEnvelopeContract: 'blocked',
      envelopeNormalized: false,
      diagnostics: [
        createDiagnostic('LIFECYCLE-TRANSACTION-001', {
          stage: 'third-party.android-response-delivery-sink.blocked-source',
          severity: 'error',
          packageId,
    ...
    const skipped = await deliverThirdPartyDataPackAndroidResponseDeliverySinkAdapter({
      platformSplitContract: skippedSource
    })
    const blocked = await deliverThirdPartyDataPackAndroidResponseDeliverySinkAdapter({
      platformSplitContract: blockedSource
    })

    expect(skipped.status).toBe('skipped')
    expect(skipped.androidResponseDeliveryAttempted).toBe(false)
    expect(skipped.androidResponseDelivered).toBe(false)
    expect(skipped.deliveryEnvelope).toBeUndefined()
    expect(blocked.status).toBe('blocked')
    expect(blocked.androidResponseDeliveryAttempted).toBe(false)
  ...
  it('blocks missing host, wrong channel, missing Android contract, missing envelope and upstream effect drift', async () => {
    const source = createPlatformSplitContract()
    const cases = [
      {
        label: 'missing host',
        source,
        host: undefined,
        blockedCheckId: 'android-sink-host-ready'
      },
      {
        label: 'wrong channel',
        source,
        ...
        blockedCheckId: 'fixed-android-channel'
      },
      {
        label: 'missing Android platform adapter',
        source: {
          ...source,
          platformAdapters: source.platformAdapters.filter(adapter => adapter.platform !== 'android')
        },
        host: {
          channel: 'android-native-response-event-sink' as const,
          deliver: (envelope: ThirdPartyDataPackUiIpcResultEnvelope) => createAcknowledgement(envelope)
        },
        blockedCheckId: 'android-platform-contract-present'
      },
      {
    ...
    for (const currentCase of cases) {
      ...
      expect(result.reason).toBe('Android response delivery sink inputs are inconsistent')
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({
          code: 'LIFECYCLE-TRANSACTION-001',
          stage: `third-party.android-response-delivery-sink.checks.${currentCase.blockedCheckId}`
        })
      ]))
  ...
  it('blocks host failures and invalid acknowledgements without leaking host paths', async () => {
    const source = createPlatformSplitContract()
    const hostFailure = await deliverThirdPartyDataPackAndroidResponseDeliverySinkAdapter({
      platformSplitContract: source,
      host: {
        channel: 'android-native-response-event-sink',
        deliver: () => {
          throw new Error('EACCES C:/Users/LENOVO/mods/android-response-delivery')
        }
      }
    })
    const invalidAcknowledgement = await deliverThirdPartyDataPackAndroidResponseDeliverySinkAdapter({
      platformSplitContract: source,
      host: {
        channel: 'android-native-response-event-sink',
        deliver: envelope => ({
    ...
    expect(hostFailure.reason).toBe('Android response delivery sink host failed before acknowledgement')
    expect(hostFailure.androidResponseDeliveryAttempted).toBe(true)
    expect(hostFailure.androidResponseDelivered).toBe(false)
    ...
    expect(invalidAcknowledgement.reason).toBe('Android response delivery sink host returned an invalid acknowledgement')
    expect(invalidAcknowledgement.androidResponseDeliveryAttempted).toBe(true)
    expect(invalidAcknowledgement.androidResponseDelivered).toBe(false)
  ...
  it('copies package summaries and diagnostics without reading hostile proxy lengths', async () => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/android-sink-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/android-sink-diagnostic-prototype')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.android-response-delivery-sink.proxy-test',
    ...
    expect(JSON.stringify(result)).not.toContain('android-sink-selected-package-ids')
  ...
  it('blocks accessor-based upstream effects without invoking path-bearing getters', async () => {
    ...
    Object.defineProperty(hostileEffects, 'androidUiResponsePublished', {
      enumerable: true,
      get() {
        effectGetterRead = true
        throw new Error('C:/Users/LENOVO/mods/android-sink-effect-getter')
      }
    })
    const source = {
