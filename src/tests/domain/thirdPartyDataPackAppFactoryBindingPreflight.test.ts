...
import {
  ...
  THIRD_PARTY_DATA_PACK_APP_FACTORY_BINDING_PREFLIGHT_KIND,
  THIRD_PARTY_DATA_PACK_APP_FACTORY_BINDING_PREFLIGHT_MODE,
  type ThirdPartyDataPackAppFactoryBindingPreflightResult
...
const createAppBootstrapWiringPreflight = (
  platform: ThirdPartyDataPackLauncherBoundaryPlatform = 'electron',
  overrides: Partial<ReturnType<typeof buildThirdPartyDataPackAppBootstrapWiringPreflight>> = {}
) => ({
  ...buildThirdPartyDataPackAppBootstrapWiringPreflight({
    normalStartupGatePreflight: createReadyNormalStartupGate(platform)
  }),
  ...overrides
})
...
const expectNoFactoryBindingRuntimeOrWriteEffects = (
  result: ThirdPartyDataPackAppFactoryBindingPreflightResult,
  prepared: boolean
): void => {
  expect(result.appFactoryBindingAllowed).toBe(false)
  ...
  expect(result.effects.appBootstrapWiringPreflightConsumed).toBe(prepared)
  expect(result.effects.appFactoryBindingPreflightPrepared).toBe(prepared)
  expect(result.effects.launcherAppFactoryBindingReportPrepared).toBe(prepared)
  expect(result.effects.gameAppFactoryBindingReportPrepared).toBe(prepared)

  const {
    appBootstrapWiringPreflightConsumed: _appBootstrapWiringPreflightConsumed,
    appFactoryBindingPreflightPrepared: _appFactoryBindingPreflightPrepared,
    launcherAppFactoryBindingReportPrepared: _launcherAppFactoryBindingReportPrepared,
    gameAppFactoryBindingReportPrepared: _gameAppFactoryBindingReportPrepared,
    ...realEffects
  } = result.effects
  expect(Object.values(realEffects).every(value => value === false)).toBe(true)
}

describe('third-party app factory binding preflight', () => {
  it('prepares a path-free factory binding report without invoking app factories', () => {
    const result = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: createAppBootstrapWiringPreflight()
    })

    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_APP_FACTORY_BINDING_PREFLIGHT_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_APP_FACTORY_BINDING_PREFLIGHT_MODE)
    expect(result.platform).toBe('electron')
    expect(result.status).toBe('deferred')
    expect(result.appFactoryBindingPreflight).toBe('deferred')
    expect(result.appBootstrapWiringPreflightStatus).toBe('deferred')
    expect(result.startupGateDecision).toBe('ready-for-launcher-boundary')
    expect(result.appBootstrapWiringPreflightConsumed).toBe(true)
    expect(result.appFactoryBindingPreflightPrepared).toBe(true)
    expect(result.launcherAppFactoryBindingReportPrepared).toBe(true)
    expect(result.gameAppFactoryBindingReportPrepared).toBe(true)
    ...
    expect(result.appFactoryBindingStages.map(stage => [stage.id, stage.status])).toEqual([
      ['app-bootstrap-wiring-preflight-consumed', 'satisfied'],
      ['app-factory-binding-preflight', 'satisfied'],
      ['launcher-app-factory-binding-report', 'deferred'],
      ['game-app-factory-binding-report', 'deferred'],
    ...
    expect(result.appFactoryBindingRequirements.map(requirement => requirement.id)).toEqual([
    ...
    expectNoFactoryBindingRuntimeOrWriteEffects(result, true)
    expectJsonGraphFrozen(result)
  })

  it('accepts Web and Android app bootstrap wiring as factory binding inputs', () => {
    const web = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: createAppBootstrapWiringPreflight('web')
    })
    const android = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: createAppBootstrapWiringPreflight('android')
    })
  ...
  it('skips and blocks terminal app bootstrap wiring results without factory requirements', () => {
    const skippedSource = buildThirdPartyDataPackAppBootstrapWiringPreflight({
      normalStartupGatePreflight: createReadyNormalStartupGate('electron', {
        status: 'skipped',
    ...
    const blockedSource = buildThirdPartyDataPackAppBootstrapWiringPreflight({
      normalStartupGatePreflight: createReadyNormalStartupGate('electron', {
        status: 'blocked',
        normalStartupGatePreflight: 'blocked',
        reason: 'app bootstrap wiring blocked upstream',
        diagnostics: [
          {
            code: 'LIFECYCLE-TRANSACTION-001',
            ruleId: 'LIFECYCLE-TRANSACTION-001',
            severity: 'error',
            stage: 'third-party.app-factory.blocked-source',
    ...
    const skipped = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: skippedSource
    })
    const blocked = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: blockedSource
    })

    expect(skipped.status).toBe('skipped')
    expect(skipped.appFactoryBindingRequirements).toEqual([])
    expect(skipped.checks.every(check => check.status === 'skipped')).toBe(true)
    expect(blocked.status).toBe('blocked')
    expect(blocked.appFactoryBindingRequirements).toEqual([])
    expect(blocked.appFactoryBindingStages.every(stage => stage.status === 'blocked')).toBe(true)
    ...
    expectNoFactoryBindingRuntimeOrWriteEffects(skipped, false)
    expectNoFactoryBindingRuntimeOrWriteEffects(blocked, false)
    expectJsonGraphFrozen(skipped)
    expectJsonGraphFrozen(blocked)
  })

  it('blocks forbidden factories and bootstrap effect drift before factory binding is executed', () => {
    const result = buildThirdPartyDataPackAppFactoryBindingPreflight({
      appBootstrapWiringPreflight: {
        ...createAppBootstrapWiringPreflight(),
        ...
        effects: {
          ...createAppBootstrapWiringPreflight().effects,
          routerMounted: true
        }
      } as unknown as ReturnType<typeof buildThirdPartyDataPackAppBootstrapWiringPreflight>
    })

    expect(result.status).toBe('blocked')
    expect(result.reason).toBe('app factory binding preflight inputs are inconsistent')
    expect(result.checks).toEqual(expect.arrayContaining([
      expect.objectContaining({
        id: 'path-free-app-bootstrap-wiring',
        status: 'blocked'
      }),
      expect.objectContaining({
        id: 'no-factory-binding-runtime-or-write-effects',
        status: 'blocked'
      })
    ]))
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        stage: 'third-party.app-factory-binding-preflight.checks.path-free-app-bootstrap-wiring'
      }),
      expect.objectContaining({
        stage: 'third-party.app-factory-binding-preflight.checks.no-factory-binding-runtime-or-write-effects'
      })
    ]))
    expect(JSON.stringify(result)).not.toContain('C:/Users')
    expect(result.effects.routerMounted).toBe(false)
    expectNoFactoryBindingRuntimeOrWriteEffects(result, false)
    expectJsonGraphFrozen(result)
  })
  ...
  it('copies package summaries and diagnostics without reading hostile accessors', () => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/factory-binding-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/factory-binding-diagnostic')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.app-factory.proxy-test',
    ...
    expect(serialized).not.toContain('factory-binding-selected-package-ids')
    expectNoFactoryBindingRuntimeOrWriteEffects(result, true)
    expectJsonGraphFrozen(result)
  })
})
