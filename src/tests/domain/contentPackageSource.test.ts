...
describe('content package source contract', () => {
  ...
  it('rejects directory array-like metadata before reading length, indexes or iterators', () => {
    let lengthRead = false
    let entryRead = false
    let iteratorRead = false
    const directoryEntries = {}
    Object.defineProperty(directoryEntries, 'length', {
      enumerable: true,
      get() {
        lengthRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/directory-array-like-length')
      }
    })
    Object.defineProperty(directoryEntries, '0', {
      enumerable: true,
      get() {
        entryRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/directory-array-like-entry')
      }
    })
    Object.defineProperty(directoryEntries, Symbol.iterator, {
      enumerable: true,
      get() {
        iteratorRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/directory-array-like-iterator')
      }
    })
    ...
    expect(error).toMatchObject({
      code: 'SOURCE_ENTRY_UNSAFE',
      message: 'Content package source directory entries metadata must be an array'
    })
    ...
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('directory-array-like')
  })
  ...
  it('rejects non-number optional archive compressed-size metadata in normalized path order before coercion or later size reads', () => {
    ...
    const hostileCompressedSize = {
      valueOf() {
        ...
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-compressed-size-valueOf')
      },
      [Symbol.toPrimitive]() {
        compressedSizeCoerced = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-compressed-size-to-primitive')
      }
    }
    const invalidCompressedEntry = {
      path: 'm-pack/manifest.json',
    ...
    const laterArchiveEntry = {
      ...
      get uncompressedSizeBytes() {
        laterUncompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-later-uncompressed-size')
      },
      get compressedSizeBytes() {
        laterCompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-later-compressed-size')
      }
    }
    ...
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('archive-sorted-non-number')
  })

  it('rejects non-number required archive uncompressed-size metadata in normalized path order before coercion or later size reads', () => {
    let uncompressedSizeCoerced = false
    let invalidCompressedSizeRead = false
    let laterUncompressedSizeRead = false
    let laterCompressedSizeRead = false
    const hostileUncompressedSize = {
      valueOf() {
        uncompressedSizeCoerced = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-uncompressed-size-valueOf')
      },
      [Symbol.toPrimitive]() {
        uncompressedSizeCoerced = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-uncompressed-size-to-primitive')
      }
    }
    const invalidUncompressedEntry = {
      path: 'm-pack/manifest.json',
      ...
      get compressedSizeBytes() {
        invalidCompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-uncompressed-compressed-size')
      }
    }
    const laterArchiveEntry = {
      path: 'z-pack/manifest.json',
      get uncompressedSizeBytes() {
        laterUncompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-uncompressed-later-uncompressed-size')
      },
      get compressedSizeBytes() {
        laterCompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-non-number-uncompressed-later-compressed-size')
      }
    }
    ...
    expect(uncompressedSizeCoerced).toBe(false)
    expect(invalidCompressedSizeRead).toBe(false)
    expect(laterUncompressedSizeRead).toBe(false)
    expect(laterCompressedSizeRead).toBe(false)
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('archive-sorted-non-number-uncompressed')
  })
  ...
  it('rejects non-normalized archive paths before reading earlier or later size metadata', () => {
    const readState = {
      earlierUncompressed: false,
      earlierCompressed: false,
      invalidUncompressed: false,
    ...
    const earlierEntry = {
      ...
      get uncompressedSizeBytes() {
        ...
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-earlier-size')
      },
      get compressedSizeBytes() {
        readState.earlierCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-earlier-compressed-size')
      }
    }
    const nonNormalizedEntry = {
      path: 'm-private-pack/./manifest.json',
      get uncompressedSizeBytes() {
        readState.invalidUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-invalid-size')
      },
      get compressedSizeBytes() {
        readState.invalidCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-invalid-compressed-size')
      }
    }
    const laterEntry = {
      get path() {
        readState.laterPath = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-later-path')
      },
      get uncompressedSizeBytes() {
        readState.laterUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-later-size')
      },
      get compressedSizeBytes() {
        readState.laterCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-nonnormalized-later-compressed-size')
      }
    }
    ...
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('archive-nonnormalized')
    expect(JSON.stringify(error)).not.toContain('m-private-pack')
  })
  ...
  it('rejects hidden archive path metadata before reading earlier sizes or later metadata', () => {
    const readState = {
      earlierUncompressed: false,
      earlierCompressed: false,
      hiddenPath: false,
    ...
    const earlierEntry = {
      ...
      get uncompressedSizeBytes() {
        readState.earlierUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-earlier-size')
      },
      get compressedSizeBytes() {
        readState.earlierCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-earlier-compressed-size')
      }
    }
    const invalidEntry = {}
    Object.defineProperty(invalidEntry, 'path', {
      enumerable: false,
      get() {
        readState.hiddenPath = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path')
      }
    })
    Object.defineProperty(invalidEntry, 'uncompressedSizeBytes', {
      ...
      get() {
        readState.invalidUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-invalid-size')
      }
    })
    Object.defineProperty(invalidEntry, 'compressedSizeBytes', {
      ...
      get() {
        readState.invalidCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-invalid-compressed-size')
      }
    })
    const laterEntry = {
      get path() {
        readState.laterPath = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-later-path')
      },
      get uncompressedSizeBytes() {
        readState.laterUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-later-size')
      },
      get compressedSizeBytes() {
        readState.laterCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-path-later-compressed-size')
      }
    }
    ...
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('archive-hidden-path')
  })

  it('rejects hidden required archive uncompressed-size metadata before reading earlier sizes or later metadata', () => {
    const readState = {
      earlierUncompressed: false,
      earlierCompressed: false,
      invalidPath: false,
      hiddenUncompressed: false,
      invalidCompressed: false,
      laterPath: false,
      laterUncompressed: false,
      laterCompressed: false
    ...
    const earlierEntry = {
      ...
      get uncompressedSizeBytes() {
        readState.earlierUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-earlier-size')
      },
      get compressedSizeBytes() {
        readState.earlierCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-earlier-compressed-size')
      }
    }
    const invalidEntry = {}
    Object.defineProperty(invalidEntry, 'path', {
      ...
      get() {
        readState.invalidPath = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-path')
      }
    })
    Object.defineProperty(invalidEntry, 'uncompressedSizeBytes', {
      enumerable: false,
      get() {
        readState.hiddenUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-size')
      }
    })
    Object.defineProperty(invalidEntry, 'compressedSizeBytes', {
      enumerable: true,
      get() {
        readState.invalidCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-compressed-size')
      }
    })
    const laterEntry = {
      get path() {
        readState.laterPath = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-later-path')
      },
      get uncompressedSizeBytes() {
        readState.laterUncompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-later-size')
      },
      get compressedSizeBytes() {
        readState.laterCompressed = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-hidden-uncompressed-later-compressed-size')
      }
    }
  ...
  it('rejects archive path-prefix conflicts in normalized path order before reading sorted-later sizes', () => {
    let conflictSizeReadCount = 0
    let laterUncompressedSizeRead = false
    let laterCompressedSizeRead = false
    const createConflictArchiveEntry = (archivePath: string) => ({
      path: archivePath,
      get uncompressedSizeBytes() {
        conflictSizeReadCount += 1
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-prefix-size')
      },
      get compressedSizeBytes() {
        conflictSizeReadCount += 1
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-prefix-compressed-size')
      }
    })
    const laterArchiveEntry = {
      path: 'z-pack/manifest.json',
      get uncompressedSizeBytes() {
        laterUncompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-prefix-later-uncompressed-size')
      },
      get compressedSizeBytes() {
        laterCompressedSizeRead = true
        throw new Error('EACCES: stat C:/Users/LENOVO/mods/archive-sorted-prefix-later-compressed-size')
      }
    }

    const error = captureSourceError(() => validateContentPackageSourceArchiveEntries([
      laterArchiveEntry,
      createConflictArchiveEntry('m-pack/data/manifest.json'),
      { path: 'a-safe.bin', uncompressedSizeBytes: 0, compressedSizeBytes: 0 },
      createConflictArchiveEntry('m-pack/data')
    ]))
    ...
    expect(conflictSizeReadCount).toBe(0)
    expect(laterUncompressedSizeRead).toBe(false)
    expect(laterCompressedSizeRead).toBe(false)
    expect(JSON.stringify(error)).not.toContain('C:/Users')
    expect(JSON.stringify(error)).not.toContain('LENOVO')
    expect(JSON.stringify(error)).not.toContain('archive-sorted-prefix')
    expect(JSON.stringify(error)).not.toContain('m-pack')
  })
