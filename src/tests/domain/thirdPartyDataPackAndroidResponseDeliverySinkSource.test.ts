...
import {
  ...
  THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_SINK_SOURCE_KIND,
  THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_SINK_SOURCE_MODE,
  ThirdPartyDataPackAndroidResponseDeliverySinkBlockedError,
  type ThirdPartyDataPackAndroidResponseDeliverySinkSourceResult
} from '@/domain/mods/thirdPartyDataPackAndroidResponseDeliverySinkSource'
...
const expectNoRuntimeOrPersistentEffects = (
  result: ThirdPartyDataPackAndroidResponseDeliverySinkSourceResult,
  delivered: boolean
): void => {
  ...
  expect(result.androidUiBridgeAllowed).toBe(delivered)
  expect(result.androidResponseDeliveryAllowed).toBe(delivered)
  ...
  if (delivered) expect(result.effects.androidSinkHostCalled).toBe(true)
  expect(result.effects.deliveryAcknowledgementReceived).toBe(delivered)
  expect(result.effects.androidResponseDelivered).toBe(delivered)
  ...
  expect(result.effects.androidUiResponsePublished).toBe(delivered)
...
describe('third-party Android response delivery sink source', () => {
  it('is disabled by default and does not call platform split or Android sink host sources', async() => {
    ...
    const source = createThirdPartyDataPackAndroidResponseDeliverySinkSource({
    ...
    expect(result.kind).toBe(THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_SINK_SOURCE_KIND)
    expect(result.mode).toBe(THIRD_PARTY_DATA_PACK_ANDROID_RESPONSE_DELIVERY_SINK_SOURCE_MODE)
    expect(result.platform).toBe('android')
    expect(result.status).toBe('skipped')
    expect(result.enabled).toBe(false)
    expect(result.sourceCalled).toBe(false)
    expect(result.androidSinkAdapterCalled).toBe(false)
  ...
  it('delivers through the injected fixed Android sink host when explicitly enabled', async() => {
    ...
    expect(result.androidSinkAdapterCalled).toBe(true)
    ...
    expect(result.androidResponseDeliverySinkAdapterStatus).toBe('delivered')
  ...
  it('allows continuation when an enabled platform split source reports skipped', async() => {
    ...
    expect(result.androidResponseDeliverySinkAdapterStatus).toBe('skipped')
    expect(host.deliver).not.toHaveBeenCalled()
  ...
  it('blocks missing sources, host failures and invalid acknowledgements without leaking host paths', async() => {
    const missingSource = createThirdPartyDataPackAndroidResponseDeliverySinkSource({
      enabled: true,
      host: {
    ...
    const throwingSource = createThirdPartyDataPackAndroidResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => {
        throw new Error('EACCES: C:/Users/LENOVO/mods/android-source-platform-split')
      },
      host: {
        channel: 'android-native-response-event-sink',
        deliver: envelope => createAcknowledgement(envelope)
    ...
    const hostFailureSource = createThirdPartyDataPackAndroidResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => createPlatformSplitContract(),
      host: {
        channel: 'android-native-response-event-sink',
        deliver: () => {
          throw new Error('EACCES C:/Users/LENOVO/mods/android-source-host')
        }
      }
    })
    const invalidAcknowledgementSource = createThirdPartyDataPackAndroidResponseDeliverySinkSource({
      enabled: true,
      readUiIpcResponseDeliveryPlatformSplitContract: async() => createPlatformSplitContract(),
      host: {
        channel: 'android-native-response-event-sink',
    ...
    await expect(missingSource()).rejects.toBeInstanceOf(ThirdPartyDataPackAndroidResponseDeliverySinkBlockedError)

    for (const currentSource of [throwingSource, hostFailureSource, invalidAcknowledgementSource]) {
      try {
        await currentSource()
      } catch (error) {
        expect(error).toBeInstanceOf(ThirdPartyDataPackAndroidResponseDeliverySinkBlockedError)
        const result = (error as ThirdPartyDataPackAndroidResponseDeliverySinkBlockedError).result
  ...
  it('blocks unsafe platform split drift while copying hostile arrays safely', async() => {
    ...
    const createHostileArray = <T>(values: T[], label: string): T[] =>
      new Proxy(values, {
        get(target, property, receiver) {
          if (property === 'length') {
            lengthRead = true
            throw new Error(`EACCES: stat C:/Users/LENOVO/mods/android-source-${label}`)
          }
    ...
    const diagnostic = Object.assign(Object.create({
      get hostPath() {
        throw new Error('C:/Users/LENOVO/mods/android-source-diagnostic-prototype')
      }
    }), {
      code: 'CACHE-INVALID-001',
      ruleId: 'CACHE-INVALID-001',
      severity: 'warning',
      stage: 'third-party.android-response-delivery-sink-source.proxy-test',
    ...
    const hostilePlatformSplit = {
      ...
      effects: {
        ...
        androidUiResponsePublished: true
      }
    ...
    try {
      ...
      expect(result.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({
          stage: 'third-party.android-response-delivery-sink-source.delivery-blocked'
        })
      ]))
      const serialized = JSON.stringify(result)
      expect(serialized).not.toContain('C:/Users')
      expect(serialized).not.toContain('LENOVO')
      expect(serialized).not.toContain('android-source-selected-package-ids')
