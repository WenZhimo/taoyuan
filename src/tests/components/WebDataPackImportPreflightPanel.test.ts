...
const createReadyElectronOrdinaryInstallTerminalContinuationResult = (
  ...
  return {
    ...
    settingsLockfileLifecycle: {
      ...
      status: 'ready',
      reason: 'Electron continuation wrote package files, settings, and mod-lock before visible panel runtime publication',
      readOnly: false,
      enabled: true,
      appBootstrapContinuationAllowed: true,
      commandContinuationAllowed: true,
...
describe('WebDataPackImportPreflightPanel', () => {
  ...
  it('disables and re-enables an installed package from the visible management list', async() => {
    ...
    try {
      ...
      expect(wrapper.get('[data-testid="web-mod-import-status"]').text()).toBe('已恢复')
      expect(wrapper.get('[data-testid="web-mod-enable-result"]').text()).toContain('启用事务：已完成')
      expect(wrapper.get('[data-testid="web-mod-enable-result"]').text()).toContain('settings 已写入')
  ...
  it('uninstalls an enabled package directly from the visible management list', async() => {
    const packageId = 'web_panel_uninstall_enabled_visible'
    const officialRegistrySet = buildOfficialRegistrySetFromStaticData()
    officialRegistrySet.freezeEntries()
    publishOfficialContentRegistrySet(officialRegistrySet)
    publishMountedAppStartupHostEvidence()
