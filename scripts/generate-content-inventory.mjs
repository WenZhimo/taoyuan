...
const reviewedArtifacts = [
  ...
  },
  {
    ...
    migrationPhase: [7],
    status: 'verified',
    rationale: 'Consumes the read-only Transaction Preflight and produces a runtime platform adapter gate report that keeps runtime enablement deferred, copies upstream official/candidate identity summaries and diagnostic objects including relatedPackageIds and nested JSON details before exposure, enumerates missing Electron, Web, Android, shared core and storage-isolation adapters, and performs no IPC exposure, import persistence, registry, package, lockfile, settings, save, cache or transaction-log writes.'
  },
  {
  ...
  },
  {
    ...
    migrationPhase: [7],
    status: 'verified',
    rationale: 'Covers deferred, skipped and blocked runtime adapter gate outcomes, required Electron/Web/Android/shared adapter enumeration, absence of candidate artifact exposure, upstream official/candidate identity summary and diagnostic object copy boundaries, deterministic repeated output, read-only protection for reports/files/userdata/settings and unchanged official 54/4242 five-hash baseline.'
  },
  {
