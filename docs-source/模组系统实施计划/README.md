...
本套文档指导《桃源乡》从编译期静态数据演进为“挂载器加载官方内容包，并在 Web/Electron 支持第三方模组”的架构。实施必须保持现有玩法、旧存档、Web/Electron 模组边界和 Android 原版构建可用，并通过可回滚的小切片推进。
...
- Android 当前只要求原版可游玩：随 APK 官方内容必须可加载，现有玩法和旧存档兼容不能破坏；外部数据包、后续模组、插件、模组管理、lockfile/事务写入和 Android 平台来源适配不属于 Android 目标，除非后续重新明确授权。
...
阶段 0 至阶段 5 已完成当前范围的实现与自动化验收；阶段 6 的低耦合纯数据候选、业务静态读取违规、机器盘点暂定状态和快照恢复顺序差异均已清零，并已完成总验收差距审计、官方内容启动门禁、正式预编译产物、`environmentHash` 绑定、失败回退和真实 Web/Electron 产物运行时探针。六项验收当前为 4 项 `PASS`、1 项 `PARTIAL`、1 项 `BLOCKED`，因此阶段 6 仍为进行中，不能宣布“官方内容包完全接管”。各阶段切片、审计矩阵、验证证据和提交详见 [执行状态](./执行状态.md)：
...
15. 阶段 6 总验收差距审计新增 `pnpm run mod:audit-phase6` 和全注册表快照恢复测试；完成消费者、机器盘点和 v2 快照 codec 收口后，477 条业务运行时数据导入已逐项归为 87 条官方适配器叶子、150 条兼容回退、190 条框架算法和 50 条合法派生，待迁移违规为 0，`businessStaticReads` 为 `PASS`。629 个导出符号均已有最终状态，其中阶段 6 的 574 个符号为 384 个 `verified` 和 190 个 `framework-retained`，暂定状态为 0；完整构建/快照恢复顺序差异由 50 降为 0。Windows、Linux、macOS 预编译逐字节检查与哈希探针以及真实 Web/Electron 运行时探针均已通过；Android 原版 APK 运行时探针和完整人工回归尚未闭合，且不再代表 Android 模组支持验收。
...
32. 官方预编译注册表切片提交确定性 JSON 产物和元数据，使用 TypeBox 契约校验 `unknown` 输入，并将 v2 快照绑定到包内容、配置、Schema 集、编译器、加载顺序和信任策略构成的 `environmentHash`。启动时优先恢复预编译产物，缺失、非法 JSON、结构/哈希错误、旧格式或环境变化均完整回退静态构建；旧静态路径和所有公开查询签名保留。Windows、Linux、macOS CI 使用同一已提交产物作为基准且全部通过，不维护平台特例。
33. 真实产物探针切片在 Web `docs/` 与 Windows 未压缩 Electron EXE 中验证实际加载产物的五个哈希、54 / 4242、注册顺序和公开查询；缺失、损坏和环境变化均完整静态回退并到达主菜单。Electron 同时验证程序目录 `userdata/`、设置/Chromium 存储和 `startup.log`，故障场景隔离运行且不修改正式数据。Android 后续仅需按原版官方内容 smoke 验证真实 APK，不需要验证外部数据包或后续模组。
