...
15. 阶段 6 总验收差距审计新增 `pnpm run mod:audit-phase6` 和全注册表快照恢复测试；连续收口炸弹、行会商品、成就物品目录和矿洞宝箱装备四组消费者后，499 条业务运行时数据导入已逐项归为 87 条官方适配器叶子、289 条兼容回退、82 条框架算法、19 条合法派生和 22 条待迁移违规。另有 151 个暂定状态符号，以及 50 个注册表的完整构建/快照恢复顺序差异；三端运行时哈希和完整人工回归仍未闭合。
